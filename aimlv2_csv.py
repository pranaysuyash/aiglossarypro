...
# Using your requested model for cost optimization
...
def save_csv_data(filename, headers, rows):
    """Save CSV data with proper encoding."""
    try:
        import pandas as pd
        df = pd.DataFrame(rows, columns=headers)
        df.to_csv(filename, index=False, encoding='utf-8')
        logger.info(f"Saved CSV: {len(rows)} rows × {len(headers)} columns")
    except Exception as e:
