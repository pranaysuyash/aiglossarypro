...
export function registerAdminRevenueRoutes(app: Express): void {
      ...
      try {
        ...
        const dashboard = {
          overview: {
            ...
            averageOrderValue:
              totalPurchases > 0 ? Math.round((totalRevenue / totalPurchases) * 100) / 100 : 0,
          },
          revenueByCurrency,
          dailyRevenue,
          period,
        ...
        logger.error('Error fetching revenue dashboard', {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        logger.error('Error exporting revenue data', {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
        res.status(500).json({
  ...
  app.get(
    '/api/admin/revenue/webhook-status',
    authenticateFirebaseToken,
    requireFirebaseAdmin,
    async (_req: Request, res: Response) => {
      try {
        ...
        logger.error('Error fetching webhook status', {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
        res.status(500).json({
  ...
  app.post(
    '/api/admin/revenue/verify-purchase',
    authenticateFirebaseToken,
    requireFirebaseAdmin,
    async (req: Request, res: Response) => {
      try {
        ...
        logger.error('Error verifying purchase', {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
        res.status(500).json({
