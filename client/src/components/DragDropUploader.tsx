...
export default function DragDropUploader({
  ...
  const getStatusBadge = useCallback((status: FileUpload['status']) => {
    switch (status) {
      ...
      case 'completed':
        return <Badge variant="outline" className="text-green-600"><Check className="h-3 w-3 mr-1" />Complete</Badge>;
      case 'error':
        ...
        <CardContent className="p-6">
          ...
          >
            ...
            <p className="text-xs text-gray-400 mb-4">
              Supported formats: {acceptedTypes.join(', ')} • Max size: {Math.round(maxFileSize / (1024 * 1024))}MB • Max files: {maxFiles}
            </p>
