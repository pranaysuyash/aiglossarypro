...
import { 
  Home, FileText, Brain, BarChart3, Settings, ChevronRight, 
  ChevronDown, Layers, Bot, Activity, Users, Shield, Zap 
} from 'lucide-react';
...
export default function AdminSidebar({
  ...
  return (
    <div className={`bg-white border-r flex-shrink-0 transition-all duration-300 ${expanded ? 'w-64' : 'w-16'}`}>
      {/* Header */}
                ...
                {section.items.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded transition-colors ${
                      activeView === item.id ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
