...
interface DiscoveryEvent {
  eventType: 'view' | 'filter_change' | 'node_click' | 'relationship_explore' | 'search' | 'layout_change';
...
export function useDiscoveryAnalytics(userId?: string) {
  ...
  const trackEvent = useCallback(async (event: DiscoveryEvent) => {
    try {
      ...
      const session: DiscoverySession = sessionData ? JSON.parse(sessionData) : {
        sessionId,
        startTime: new Date(),
        events: [],
      ...
      localStorage.setItem('discoverySession', JSON.stringify({
        ...session,
  ...
  const trackFilterChange = useCallback((filters: any, previousFilters: any) => {
  ...
  const getRecommendations = useCallback(async (currentNodeId?: string) => {
    try {
      const sessionData = localStorage.getItem('discoverySession');
