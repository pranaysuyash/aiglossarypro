...
export function useErrorHandler() {
  ...
  const handleError = useCallback((
    ...
    } else if (normalizedError.message.includes('401') || normalizedError.message.includes('unauthorized')) {
      errorMessage = ERROR_MESSAGES.SESSION_ERROR;
    } else if (normalizedError.message.includes('403') || normalizedError.message.includes('forbidden')) {
      errorMessage = ERROR_MESSAGES.PERMISSION_ERROR;
    } else if (normalizedError.message.includes('429') || normalizedError.message.includes('rate limit')) {
