...
export const useGestureNavigation = (settings: Partial<GestureSettings> = {}) => {
  ...
  const calculateGesture = useCallback((start: TouchPoint, end: TouchPoint): SwipeGesture | null => {
    ...
    if (distance < config.swipeThreshold || velocity < config.velocityThreshold || duration > config.maxSwipeTime) {
      return null;
    }
  ...
  const handleSwipeNavigation = useCallback((gesture: SwipeGesture) => {
    if (!config.enableSwipeNavigation) return;

    const currentPath = window.location.pathname;
    
    // Define navigation patterns
    const navigationMap: Record<string, { left?: string; right?: string; up?: string; down?: string }> = {
      '/': {
        right: '/categories',
        down: '/trending'
      },
  ...
  const getGesturePattern = useCallback(() => {
    ...
    if (directions.join('') === 'updown' || directions.join('') === 'downup') return 'shake_vertical';
    if (directions.join('') === 'leftright' || directions.join('') === 'rightleft') return 'shake_horizontal';
  ...
  const getGestureStats = useCallback(() => {
    ...
    const directionCounts = gestures.reduce((counts, g) => {
    ...
    return {
      ...
      directionCounts,
      mostCommonDirection: Object.entries(directionCounts).sort(([,a], [,b]) => b - a)[0]?.[0] as SwipeGesture['direction']
    };
  }, []);
