...
export const usePersonalizationTracking = () => {
  ...
  // Track page view
  const trackPageView = useCallback((path: string, additionalContext?: Record<string, any>) => {
    if (!isAuthenticated) return;
  ...
  // Track term view
  const trackTermView = useCallback((termId: string, termData?: any) => {
    trackEvent({
      eventType: 'view',
      entityType: 'term',
      entityId: termId,
  ...
  const trackFavorite = useCallback((termId: string, action: 'add' | 'remove') => {
    trackEvent({
      eventType: 'favorite',
      entityType: 'term',
      entityId: termId,
  ...
  const trackLearningProgress = useCallback((pathId: string, stepId?: string, progress?: number) => {
    trackEvent({
      eventType: 'progress',
  ...
  const trackContentEngagement = useCallback((contentType: string, contentId: string, engagementType: string, data?: any) => {
    trackEvent({
  ...
  const trackCategoryView = useCallback((categoryId: string, categoryData?: any) => {
    trackEvent({
      eventType: 'view',
      entityType: 'category',
      entityId: categoryId,
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollPercent > engagementData.current.scrollDepth) {
        ...
        if (scrollPercent >= 25 && scrollPercent % 25 === 0 && scrollPercent > lastScrollDepth.current) {
...
export const useComponentTracking = (componentName: string) => {
  ...
  const trackComponentEvent = useCallback((action: string, data?: any) => {
    trackEvent({
  ...
  const trackComponentView = useCallback((data?: any) => {
    trackComponentEvent('view', data);
  }, [trackComponentEvent]);

  const trackComponentInteraction = useCallback((action: string, data?: any) => {
