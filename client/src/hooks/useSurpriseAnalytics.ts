...
export function useSurpriseAnalytics() {
  ...
  const trackDiscoveryEvent = useCallback(async (event: SurpriseDiscoveryEvent) => {
    try {
  ...
  const trackModePreference = useCallback(async (mode: string, reason?: string) => {
    try {
      await fetch('/api/analytics/events', {
        method: 'POST',
        headers: {
  ...
  const trackSessionPattern = useCallback(async (
    sessionId: string,
    pattern: {
