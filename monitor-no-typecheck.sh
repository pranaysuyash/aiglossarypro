...
SERVICE_NAME="aiglossary-prod-no-typecheck"
SERVICE_URL="https://jnnkppp6sp.us-east-1.awsapprunner.com"
SERVICE_ARN="arn:aws:apprunner:us-east-1:927289246324:service/aiglossary-prod-no-typecheck/95758e2f6ff24177946400d7a507b3d3"
...
while true; do
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" == "RUNNING" ]; then
      ...
      aws logs tail /aws/apprunner/aiglossary-prod-no-typecheck/95758e2f6ff24177946400d7a507b3d3/service --since 5m 2>/dev/null || echo "Unable to fetch logs"
      break
    else
      echo ""
    fi
