...
if (specificUser) console.log(`👤 Specific User: ${specificUser}`);
...
class AggressiveInteractionAudit {
  ...
  async captureScreenshot(page: Page, name: string, fullPage: boolean = false) {
  ...
  async testAllInteractions(page: Page, pageName: string) {
  ...
  async testResponsiveDesign(page: Page, pageName: string) {
    const viewports = [
  ...
  async testDarkMode(page: Page, pageName: string) {
  ...
  async testTermInteractions(page: Page) {
  ...
  async testUserJourney(context: BrowserContext, user: TestUser) {
  ...
  async testPublicPages(context: BrowserContext) {
