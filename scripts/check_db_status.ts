...
async function checkDatabaseStatus() {
  try {
    ...
    const sectionProgress =
      sectionsResult.length > 0 ? ((sectionsResult.length / expectedSections) * 100).toFixed(1) : 0;
    console.log(
      `Expected sections for ${enhancedResult.length} enhanced terms: ${expectedSections}`
    );
