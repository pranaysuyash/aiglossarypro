...
class CIPerformanceChecker {
  ...
  async runPerformanceCheck(): Promise<PerformanceResult> {
  ...
  private async runBuildPerformanceTest(): Promise<{ buildTime: number }> {
  ...
  private async analyzeBundleSize(): Promise<{ bundleSize: number }> {
  ...
  private async runRenderPerformanceTest(): Promise<{
    averageRenderTime: number;
    maxRenderTime: number;
    slowRenderPercentage: number;
    memoryUsage: number;
  ...
  private async runReactScanAnalysis(): Promise<{
    averageRenderTime: number;
    maxRenderTime: number;
    slowRenderPercentage: number;
    memoryUsage: number;
  ...
  private parseReactScanOutput(output: string): {
    averageRenderTime: number;
    maxRenderTime: number;
    slowRenderPercentage: number;
    memoryUsage: number;
  ...
  private checkThresholds(metrics: PerformanceResult['metrics']): string[] {
    const violations: string[] = [];
    
    if (metrics.averageRenderTime > this.thresholds.averageRenderTime) {
      violations.push(
        `Average render time (${metrics.averageRenderTime.toFixed(2)}ms) exceeds threshold (${this.thresholds.averageRenderTime}ms)`
      );
    }
    
    if (metrics.maxRenderTime > this.thresholds.maxRenderTime) {
      violations.push(
        `Maximum render time (${metrics.maxRenderTime.toFixed(2)}ms) exceeds threshold (${this.thresholds.maxRenderTime}ms)`
      );
    }
    
    if (metrics.slowRenderPercentage > this.thresholds.slowRenderPercentage) {
      violations.push(
        `Slow render percentage (${metrics.slowRenderPercentage.toFixed(2)}%) exceeds threshold (${this.thresholds.slowRenderPercentage}%)`
      );
    }
    
    if (metrics.memoryUsage > this.thresholds.memoryUsage) {
      violations.push(
        `Memory usage (${metrics.memoryUsage.toFixed(2)}MB) exceeds threshold (${this.thresholds.memoryUsage}MB)`
      );
    }
    
    if (metrics.bundleSize > this.thresholds.bundleSize) {
      violations.push(
        `Bundle size (${metrics.bundleSize.toFixed(2)}MB) exceeds threshold (${this.thresholds.bundleSize}MB)`
      );
    }
    
    if (metrics.buildTime > this.thresholds.buildTime) {
      violations.push(
        `Build time (${metrics.buildTime}ms) exceeds threshold (${this.thresholds.buildTime}ms)`
      );
    }
  ...
  private generateRecommendations(metrics: PerformanceResult['metrics'], violations: string[]): string[] {
  ...
  private async saveResults(result: PerformanceResult): Promise<void> {
  ...
  private outputResults(result: PerformanceResult): void {
    console.log('\n' + '='.repeat(50));
