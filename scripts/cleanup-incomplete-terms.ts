...
async function cleanupIncompleteTerms() {
  ...
  try {
    ...
    console.log(
      `   Missing definitions: ${stats.missing_definitions} (${((stats.missing_definitions / stats.total_terms) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Missing short definitions: ${stats.missing_short_definitions} (${((stats.missing_short_definitions / stats.total_terms) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Uncategorized: ${stats.uncategorized} (${((stats.uncategorized / stats.total_terms) * 100).toFixed(1)}%)`
    );
    ...
    samples.rows.forEach(term => {
      console.log(
        `   - ${term.name} (ID: ${term.id}) - ${term.def_status}, ${term.short_def_status}`
      );
    });
