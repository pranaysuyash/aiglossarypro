...
class CompleteFeatureTester {
  ...
  addResult(result: TestResult) {
    ...
    console.log(
      chalk[color](
        `${result.status}: [${result.category}] ${result.testName} (${result.userType}) - ${result.message}`
      )
    );
  }
  ...
  async dismissCookieBanner(page: any) {
    try {
      ...
      const cookieBanner = await page
        .locator(
          '[data-testid="cookie-banner"], .cookie-banner, [role="dialog"]:has-text("cookie")'
        )
        .first();
    ...
    for (const endpoint of endpoints) {
      try {
        const { stdout, stderr } = await execAsync(
          `curl -s -w "%{http_code}" -o /dev/null ${this.apiUrl}${endpoint.path}`
        );
  ...
  async testCookieConsent() {
    ...
    try {
      ...
      const cookieElements = await page
        .locator('[class*="cookie"], [data-testid*="cookie"], [aria-label*="cookie"]')
        .count();
      const posthogScript = await page.locator('script[src*="posthog"]').count();
      const gaScript = await page
        .locator('script[src*="google"], script[src*="analytics"]')
        .count();
  ...
  async testAuthentication() {
    ...
    try {
      ...
      if (testUsersTab) {
        ...
        const userTypes = await page
          .locator('text="Regular User", text="Premium User", text="Admin User"')
          .count();
  ...
  async testLoginFlow(page: any) {
    try {
      ...
      const errorMessages = await page
        .locator('[role="alert"], .error, .destructive')
        .allTextContents();
    ...
    try {
      ...
      const adminElements = {
        dashboard: await page
          .locator('text="Admin Dashboard", .admin-dashboard, [data-testid*="admin"]')
          .count(),
        generateButton: await page
          .locator('button:has-text("Generate"), button[data-testid*="generate"]')
          .count(),
      ...
      const adminFeaturesFound = Object.values(adminElements).reduce(
        (sum, count) => sum + count,
        0
      );
  ...
  async testPremiumFeatures() {
    ...
    try {
      ...
      const premiumBadges = await page
        .locator('text="Premium", text="PRO", .premium-badge')
        .count();
      const upgradePrompts = await page
        .locator('button:has-text("Upgrade"), button:has-text("Get Premium")')
        .count();
      const lockedContent = await page
        .locator('.locked, .premium-only, text="Premium Only"')
        .count();
      const sections42 = await page
        .locator('[data-testid*="section"], .section, .accordion-item')
        .count();
    ...
    for (const viewport of viewports) {
      ...
      try {
        ...
        const menuButton = await page
          .locator('button[aria-label*="menu"], .hamburger')
          .first()
          .isVisible();
  ...
  async generateReport() {
    ...
    report.categories.forEach(cat => {
      ...
      console.log(
        chalk.gray(`  ${cat.name}: ${cat.passed}/${cat.tests.length} passed (${passRate}%)`)
      );
    });
