...
const sourcePath = getCLIArg('--source') as string;
const importType = getCLIArg('--type', 'essential') as string;
const shouldEnhance = args.includes('--enhance');
const shouldValidate = args.includes('--validate');
const batchSize = parseInt(getCLIArg('--batch-size', '10') as string, 10);
const isDryRun = args.includes('--dry-run');
const targetCategory = getCLIArg('--category') as string;

// Import types
type ImportFormat = 'csv' | 'json' | 'essential' | 'excel';
 ...
 */
async function loadImportData(format: ImportFormat, sourcePath: string): Promise<ImportRecord[]> {
  switch (format) {
    case 'essential':
 ...
 * Load CSV data
 */
async function loadCSVData(csvPath: string): Promise<ImportRecord[]> {
  if (!csvPath) {
 ...
 * Load JSON data
 */
async function loadJSONData(jsonPath: string): Promise<ImportRecord[]> {
  if (!jsonPath) {
 ...
 */
async function loadExcelData(excelPath: string): Promise<ImportRecord[]> {
  if (!excelPath) {
 ...
 */
async function enhanceRecord(record: ImportRecord): Promise<void> {
  try {
    // Generate missing definition
    if (!record.definition && record.name) {
      const aiDefinition = await aiService.generateDefinition(
        record.name,
        record.category,
 ...
 */
async function importTerm(record: ImportRecord, categoryId: string | null): Promise<string> {
  const termData = {
    name: record.name,
 ...
 */
function reportImportStats(stats: ImportStats): void {
  logger.info('\n📊 BULK IMPORT RESULTS');
  logger.info('════════════════════════');
