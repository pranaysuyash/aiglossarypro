...
const generateCount = parseInt(getCLIArg('--count', '10') as string, 10);

interface TermGenerationConfig {
  category: string;
...
async function seedTerms(): Promise<void> {
  ...
  try {
    ...
    const termConfigs = await determineTermsToGenerate(
      existingCategories,
      existingTerms,
      targetCategory,
      generateCount
...
async function generateSingleTerm(
  ...
  try {
    ...
    const definition = await aiService.generateDefinition(
      suggestion.term,
      config.category,
      `Priority: ${config.priority}, Complexity: ${config.complexity}, Focus: ${config.focusAreas.join(', ')}`
    );
 ...
 */
function reportResults(stats: GenerationStats): void {
