  ...
  try {
    ...
    for (const term of termsToValidate) {
      try {
        ...
        logger.info(
          `  Score: ${result.overallScore}/100 (Q:${result.qualityScore} C:${result.completenessScore} Cons:${result.consistencyScore})`
        );
...
async function validateQuality(
  term: any
): Promise<{ score: number; issues: ValidationIssue[]; recommendations: string[] }> {
  const issues: ValidationIssue[] = [];
  const recommendations: string[] = [];
  let score = 100;
...
async function validateCompleteness(
  ...
  for (const { field, weight, message } of enrichmentFields) {
    ...
    if (isEmpty) {
      const isRequired =
        (field === 'mathFormulation' && term.category?.toLowerCase().includes('algorithm')) ||
...
async function validateConsistency(
  ...
  if (term.definition && term.shortDefinition) {
    const consistencyScore = await checkDefinitionConsistency(
      term.definition,
  ...
  if (term.category && term.definition) {
    const categoryConsistency = await checkCategoryConsistency(
      term.name,
      term.definition,
      term.category
    );
...
async function checkDefinitionConsistency(
  definition: string,
...
function displayValidationSummary(report: ValidationReport, duration: number): void {
  ...
  logger.info('\n🎯 Threshold Analysis:');
  logger.info(
    `  Above Threshold (${qualityThreshold}): ${summary.termsAboveThreshold} (${Math.round((summary.termsAboveThreshold / report.totalTerms) * 100)}%)`
  );
  logger.info(
    `  Below Threshold: ${summary.termsBelowThreshold} (${Math.round((summary.termsBelowThreshold / report.totalTerms) * 100)}%)`
  );
  ...
  logger.info(
    `  Auto-fix Rate: ${summary.totalIssues > 0 ? Math.round((summary.fixableIssues / summary.totalIssues) * 100) : 0}%`
  );
...
export { validateContent, validateTerm, type ValidationResult, type ValidationReport };
