...
const SAMPLE_TERMS: SampleTerm[] = [
  ...
  {
    name: "Neural Network",
    shortDefinition: "A computing system inspired by biological neural networks that learns to perform tasks by analyzing examples.",
  ...
  },
  {
    ...
    shortDefinition: "The ability of computers to understand, interpret, and generate human language.",
    definition: "Natural Language Processing (NLP) is a branch of artificial intelligence that deals with the interaction between computers and humans through natural language. The ultimate objective of NLP is to read, decipher, understand, and make sense of human languages in a manner that is valuable.",
    category: "NLP",
    difficulty: "intermediate",
  ...
  {
    name: "Deep Learning",
    shortDefinition: "A subset of machine learning using neural networks with multiple layers to model complex patterns.",
    definition: "Deep Learning is a subset of machine learning in artificial intelligence that has networks capable of learning unsupervised from data that is unstructured or unlabeled. Also known as deep neural learning or deep neural network, it mimics the workings of the human brain in processing data and creating patterns for use in decision making.",
    category: "Deep Learning",
    difficulty: "advanced",
  ...
  },
  {
    ...
    shortDefinition: "The field of AI that enables computers to interpret and understand visual information from the world.",
...
const DEFAULT_CATEGORIES = [
  { name: "Core AI Concepts", description: "Fundamental artificial intelligence concepts", slug: "core-ai-concepts" },
  { name: "Machine Learning", description: "Machine learning algorithms and techniques", slug: "machine-learning" },
  { name: "Deep Learning", description: "Deep neural networks and advanced architectures", slug: "deep-learning" },
  { name: "Natural Language Processing", description: "NLP techniques and language understanding", slug: "nlp" },
  { name: "Computer Vision", description: "Visual recognition and image processing", slug: "computer-vision" },
  { name: "Reinforcement Learning", description: "Learning through interaction and rewards", slug: "reinforcement-learning" },
  { name: "Data Science", description: "Data analysis and statistical methods", slug: "data-science" },
  { name: "Statistics", description: "Statistical methods and probability theory", slug: "statistics" }
];
...
async function setupSampleTerms() {
  ...
  for (const term of SAMPLE_TERMS) {
    try {
      ...
      const [newTerm] = await db.insert(enhancedTerms).values({
        name: term.name,
...
async function validateSetup() {
  ...
  try {
    ...
    // Summary
    const totalContent = Number(termCount.rows[0].count) * Number(sectionCount.rows[0].count) / Number(termCount.rows[0].count);
    logger.info(`\n📊 Setup Summary:`);
...
if (require.main === module) {
  runCompleteSetup().then(() => process.exit(0)).catch(error => {
