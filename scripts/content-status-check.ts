...
async function checkContentStatus() {
  try {
    ...
    // Overall assessment
    const definitionRate = Math.round(
      (Number(termsStats.terms_with_definition) / Number(termsStats.total_terms)) * 100
    );
    const contentRate = Math.round(
      (Number(contentStats.substantial_content) / Math.max(Number(contentStats.total_items), 1)) *
        100
    );
