...
async function debugTerm() {
  ...
  try {
    ...
    const hasTermData = scripts.some(script => script.includes('isPreview') || script.includes('"preview"'));
    console.log(`📜 Term data in scripts: ${hasTermData ? '✅ Found' : '❌ Not found'}`);
    
    // Wait a bit more and check again
    await page.waitForTimeout(3000);
    const contentNavAfterWait = await page.locator('h2:has-text("Content Navigation")').count() > 0;
    console.log(`📋 Content Navigation after wait: ${contentNavAfterWait ? '✅ Found' : '❌ Not found'}`);
