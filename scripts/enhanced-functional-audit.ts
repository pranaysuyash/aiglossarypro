    ...
    if (this.runAccessibility && this.shouldRunAccessibilityScan(actionType)) {
      try {
        ...
        if (significantViolations.length > 0) {
          ...
          console.log(
            chalk.red(
              `♿ Found ${significantViolations.length} significant accessibility issues (${violations.length} total)`
            )
          );
        }
      } catch (error) {
      ...
      for (const navData of navItemsData) {
        ...
        try {
          ...
          await this.captureAction(
            page,
            'navigate',
            `Failed to navigate to "${navData.text}": ${error.message}`
          );
        }
      }
    } catch (error) {
  ...
  async runAudit(): Promise<void> {
    ...
    try {
      for (const user of this.testUsers) {
        ...
        const totalScreenshots = flows.reduce(
          (sum, flow) => sum + flow.actions.filter(action => action.screenshotPath).length,
          0
        );
        const accessibilityViolations = flows.reduce(
          (sum, flow) =>
            sum +
            flow.actions.reduce(
              (actionSum, action) => actionSum + (action.accessibilityIssues?.length || 0),
              0
            ),
          0
        );
  ...
  async generateReport(): Promise<void> {
    ...
    const report = {
      ...
      summary: {
        ...
        totalScreenshots: this.testResults.reduce(
          (sum, result) => sum + result.totalScreenshots,
          0
        ),
        totalAccessibilityViolations: this.testResults.reduce(
          (sum, result) => sum + result.accessibilityViolations,
          0
        ),
      },
...
export {
  EnhancedFunctionalAuditor,
  type TestFlow,
  type UserAction,
  type EnhancedFunctionalTestResult,
};
