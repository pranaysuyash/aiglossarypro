...
async function comprehensiveAudit() {
  ...
  const pages = [
    ...
    { name: 'Category Page', url: 'http://localhost:5173/category/79f3d163-dae1-499d-8371-047accbe70e9' },
    { name: 'Search Results', url: 'http://localhost:5173/terms?search=machine%20learning' },
    { name: 'Enhanced Term Detail', url: 'http://localhost:5173/enhanced/terms/8b5bff9a-afb7-4691-a58e-adc2bf94f941' }
  ];
  ...
  for (const pageInfo of pages) {
    ...
    try {
      ...
      const elements = await page.evaluate(() => {
        ...
        const searchInputs = document.querySelectorAll('input[type="text"], input[placeholder*="search"], input[placeholder*="Search"]').length;
      ...
      const hierarchicalNavigation = await page.locator('h2:has-text("Content Navigation")').count() > 0;
      ...
      const result: AuditResult = {
        ...
        features: { hierarchicalNavigation, searchFunctionality, mobileResponsive, accessibilityCompliant },
        screenshots
      };
  ...
  console.log(`   - Hierarchical Navigation found: ${report.summary.pagesWithHierarchicalNav}/${report.summary.totalPages}`);
  console.log(`   - Search functionality: ${report.summary.pagesWithSearch}/${report.summary.totalPages}`);
  console.log(`   - Pages with errors: ${report.summary.pagesWithErrors}/${report.summary.totalPages}`);
