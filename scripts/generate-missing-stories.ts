...
class StoryGenerator {
  ...
  private async scanComponents() {
    ...
    for (const pattern of componentPatterns) {
      const files = await glob(pattern, { ignore: ['**/*.stories.*', '**/*.test.*', '**/*.spec.*'] });
  ...
  private extractComponentName(filePath: string, content: string): string | null {
    ...
    if (filename.includes('.') || filename.toLowerCase().includes('util') || 
        filename.toLowerCase().includes('helper') || filename.toLowerCase().includes('hook')) {
      return null;
    }
  ...
  private async generateMissingStories() {
    ...
    const priorityOrder = ['Pages', 'UI Components', 'Admin', 'Landing', 'Components', 'Interactive', 'Analytics', 'AR/VR', 'Accessibility'];
    ...
    for (const component of prioritized) {
      if (generated >= 50) { // Limit to 50 stories per run to avoid overwhelming
