...
class QuickVisualAuditor {
  ...
  async generateReport() {
    ...
    await fs.writeFile(
      path.join(this.resultsDir, 'report.json'),
    ...
    await fs.writeFile(
      path.join(this.resultsDir, 'report.html'),
      html
    );
  }
...
<body>
  ...
  generateResultsHTML(results: any[]) {
    ...
    return Object.entries(groupedResults).map(([page, pageResults]: [string, any]) => `
      <div class="result-item">
        ...
        ${(pageResults as any[]).some(r => r.error) ? `
          <div class="error">
            ${(pageResults as any[]).filter(r => r.error).map(r => `${r.breakpoint || 'Test'}: ${r.error}`).join('<br>')}
          </div>
        ` : ''}
        ${(pageResults as any[]).some(r => r.issues && r.issues.length > 0) ? `
          <div class="issues">
            <strong>Issues found:</strong><br>
            ${(pageResults as any[]).filter(r => r.issues).map(r => 
              r.issues.map((issue: any) => `${r.breakpoint}: ${issue.type} (${issue.count || issue.issues?.length || 0})`).join('<br>')
            ).join('<br>')}
          </div>
        ` : ''}
      </div>
