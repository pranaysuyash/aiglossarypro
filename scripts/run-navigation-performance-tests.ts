...
class NavigationPerformanceTester {
  ...
  async runTest(testName: string, datasetSize: any): Promise<TestResult> {
    ...
    if (
      renderTime > PERFORMANCE_THRESHOLDS.renderTime.critical ||
  ...
  private generateReport(): PerformanceReport {
    ...
    const avgMemoryUsage =
      this.results.reduce((sum, r) => sum + r.metrics.memoryUsage, 0) / this.results.length;
  ...
  private generateRecommendations(): string[] {
    ...
    if (failedTests.length > 0) {
      recommendations.push(
        'CRITICAL: Some tests failed performance thresholds. Consider implementing virtual scrolling and component memoization.'
      );
    }

    if (warningTests.length > 0) {
      recommendations.push(
        'WARNING: Some tests exceeded warning thresholds. Monitor performance in production.'
      );
    }
    ...
    if (avgMemoryUsage > PERFORMANCE_THRESHOLDS.memoryUsage.warning) {
      recommendations.push(
        'Memory usage is elevated. Implement component cleanup and virtual scrolling.'
      );
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'All tests passed successfully. The navigation system is production-ready.'
      );
    }
...
async function main() {
  try {
    ...
    console.log(
      `📈 Summary: ${report.summary.passedTests}/${report.summary.totalTests} tests passed`
    );

    if (report.summary.failedTests > 0) {
      console.log(
        '❌ Some tests failed. Review the detailed report for optimization recommendations.'
      );
      process.exit(1);
...
export { NavigationPerformanceTester, type PerformanceReport, type TestResult };
