...
class SimpleFrontendAuditor {
  ...
  async testApiEndpoints(): Promise<TestResult[]> {
    const endpoints = ['/api/terms', '/api/categories', '/api/sections', '/api/auth/status'];
  ...
  async checkHierarchicalNavigation(): Promise<TestResult> {
    try {
      ...
      const hasHierarchicalNav =
        html.includes('HierarchicalNavigator') ||
        html.includes('Tree') ||
