...
class Quick3DPerformanceTester {
  ...
  private estimateMemoryUsage(nodeCount: number, edgeCount: number): number {
  ...
  private generateRecommendations(result: QuickTestResult): string[] {
  ...
  public async runAllTests(): Promise<QuickTestResult[]> {
  ...
  public saveResults(): void {
    ...
    try {
      writeFileSync(
        join(outputDir, '3d-performance-results.json'),
