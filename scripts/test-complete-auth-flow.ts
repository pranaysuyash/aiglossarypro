  ...
  async testCompleteAuth() {
    ...
    try {
      ...
      if (!cookieHandled) {
        console.log(
          chalk.yellow(
            '⚠️ No cookie consent button found, trying to dismiss banner by clicking outside'
          )
        );
      ...
      if ((await testTab.count()) > 0) {
        ...
        if (
          (emailValue === 'test@aimlglossary.com' && passwordValue === 'testpass123') ||
          ...
          console.log(
            chalk.yellow(
              '💡 This suggests the "Use This Account" button didn\'t populate the form properly'
            )
          );
        }
      } else {
