...
// All 295 columns from the schema - split into 42 sections
const ALL_SECTIONS = [
...
async function loadProgress() {
  const progressFile = path.join(process.cwd(), 'content-generation-tests', 'transformer-progress.json');
  try {
...
async function saveProgress(results) {
  ...
  const progressFile = path.join(outputDir, 'transformer-progress.json');
...
async function generateCompleteContent() {
  ...
  console.log('This will generate content for all sections incrementally. Progress is saved after each section.\n');
  
  // Load existing progress or initialize new
  let results = await loadProgress();
  if (!results) {
    results = {
      metadata: {
        term,
  ...
  const completedSections = Object.keys(results.content);
  const startIndex = completedSections.length;
  
  if (startIndex > 0) {
    console.log(`📊 Resuming from section ${startIndex + 1} (${completedSections.length} sections already completed)\n`);
  }

  // Generate content for remaining sections
  for (let i = startIndex; i < ALL_SECTIONS.length; i++) {
...
**Completed**: ${results.metadata.completedAt}  
**Term**: ${term}  
**Category**: machine-learning  
**Subcategory**: neural-networks  
...
`;
  ...
  console.log(`   - Progress: transformer-progress.json`);
