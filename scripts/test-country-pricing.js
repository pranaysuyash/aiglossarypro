...
function calculatePricing(countryCode, countryName) {
  ...
  return {
    ...
    totalSavings:
      basePrice - (LAUNCH_PRICING_CONFIG.isActive ? LAUNCH_PRICING_CONFIG.launchPrice : localPrice),
...
testCountries.forEach(country => {
  ...
  console.log(
    `   Total Savings:        $${pricing.totalSavings} (${Math.round((pricing.totalSavings / pricing.basePrice) * 100)}%)`
  );
  ...
  const valid =
    pricing.discount === expected.discount &&
