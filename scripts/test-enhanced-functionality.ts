  ...
  try {
    ...
    for (const route of termRoutes) {
      try {
        ...
        const titleSelectors = ['h1', 'h2', '.title', '[data-testid="term-title"]', '.term-title', '.page-title'];
        let title = '';
    ...
    if (successfulRoute) {
      try {
        ...
        const sectionElements = await page.$$eval('[data-section], .section, .term-section, [class*="section"]', 
...
async function testSearchFunctionality(page: Page): Promise<TestResult[]> {
  ...
  try {
    ...
    if (searchInput) {
      ...
      const hasSearchResults = termCards.length > 0 || resultElements.length > 0;
...
async function testCodeExamples(page: Page, results: TestResult[]) {
  try {
    ...
    const codeElements = await page.$$eval('code, pre, .code-block, .highlight, [class*="language-"]', 
      elements => elements.map(el => ({
        tag: el.tagName,
        className: el.className,
...
${allResults.filter(r => !r.success).length === 0 ? 
  ...
  allResults.filter(r => !r.success).map(r => `- ❌ ${r.test}: ${r.details}`).join('\n')
}
