...
async function testFirebaseBase64() {
  ...
  try {
    ...
    console.log(chalk.gray(`  FIREBASE_PRIVATE_KEY_BASE64: ${process.env.FIREBASE_PRIVATE_KEY_BASE64 ? 'SET (length: ' + process.env.FIREBASE_PRIVATE_KEY_BASE64.length + ')' : 'NOT SET'}`));
...
testFirebaseBase64().then(success => {
