...
class AuthFlowTester {
  ...
  async testFullAuthFlow() {
    ...
    try {
      ...
      const cookieConsent = page.locator('[role="dialog"]:has-text("Cookie"), .cookie-consent, .cookie-banner');
      ...
      if (await testTab.count() > 0) {
        ...
        if (await useAccountButton.count() > 0) {
          ...
          await page.waitForFunction(() => {
            ...
            return activeTab && activeTab.textContent?.includes('Sign In');
          }, { timeout: 5000 });
            ...
            if (await signInButton.count() > 0) {
              ...
              if (currentUrl.includes('/dashboard')) {
                ...
                if (errorMessage) {
                  ...
                  console.log(chalk.yellow('⚠️ Still on login page, checking console for errors...'));
