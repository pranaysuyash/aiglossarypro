...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const mockTest = async () => {
    ...
    try {
      const configPath = path.join(path.dirname(new URL(import.meta.url).pathname), 'visual-audit-config.ts');
      ...
      const enhancedExists = await fs.access(enhancedPath).then(() => true).catch(() => false);
