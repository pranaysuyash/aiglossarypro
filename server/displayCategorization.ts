...
export class ContentOrganizer {
  static organizeForCard(sections: Map<string, any>): any {
    ...
    DEFAULT_LAYOUT.cardContent.forEach(sectionName => {
      if (sections.has(sectionName)) {
        ...
        cardData[sectionName] = this.extractCardSummary(sectionData, sectionName);
      }
    });
  ...
  static extractFilterData(sections: Map<string, any>, categories: any): any {
    ...
    DEFAULT_LAYOUT.filterableData.forEach(sectionName => {
      if (sections.has(sectionName)) {
        const sectionData = sections.get(sectionName);
        filterData[sectionName] = this.extractFilterableValues(sectionData);
      }
    });
    
    // Determine computed filters
    filterData.difficulty_level = this.determineDifficultyLevel(sections);
    filterData.has_implementation = sections.has('Implementation') ? 'Yes' : 'No';
    filterData.has_interactive_elements = this.hasInteractiveElements(sections) ? 'Yes' : 'No';
  ...
  static extractSearchData(sections: Map<string, any>): string {
    ...
    DEFAULT_LAYOUT.searchableData.forEach(sectionName => {
      if (sections.has(sectionName)) {
        const sectionData = sections.get(sectionName);
        searchText += ' ' + this.extractSearchableText(sectionData);
      }
    });
  ...
  private static hasInteractiveElements(sections: Map<string, any>): boolean {
    ...
    for (const [sectionName, sectionData] of sections) {
      if (typeof sectionData === 'object') {
        ...
        if (dataStr.includes('interactive') || dataStr.includes('mermaid') || 
            dataStr.includes('quiz') || dataStr.includes('demo')) {
          return true;
        }
      }
    }
