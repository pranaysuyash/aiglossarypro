...
import { ColumnBatchCleanupJobData, ColumnBatchCleanupJobResult } from '../types';
  ...
  try {
    ...
    for (const operationId of uniqueOperationsToRemove) {
      ...
      if (operation) {
        ...
        estimatedSpaceFreed += 1024; // Base operation data
        estimatedSpaceFreed += operation.errors.length * 100; // Error data
        estimatedSpaceFreed += operation.subJobs.length * 50; // Sub-job references
      }
    }
    ...
    for (let i = 0; i < uniqueOperationsToRemove.length; i += batchSize) {
      ...
      for (const operationId of batch) {
        try {
          ...
          const operation = allOperations.find(op => op.id === operationId);
          if (operation) {
            logger.info(`Would remove operation ${operationId}: ${operation.sectionName} (${operation.status})`);
            result.operationsRemoved++;
            
            // Add estimated space freed for this operation
            result.spaceFreed += 1024 + (operation.errors.length * 100) + (operation.subJobs.length * 50);
          }
    ...
    try {
      ...
      const oldAlerts = allAlerts.filter(alert => 
        alert.triggeredAt < cutoffDate && alert.acknowledged
      );
