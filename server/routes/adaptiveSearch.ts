...
export function registerAdaptiveSearchRoutes(app: Express): void {
   ...
   */
  app.get('/api/adaptive-search', (req, res, next) => {
    try {
    ...
    try {
      ...
      const enhancedResults = searchResponse.results.map(result => ({
        ...
        semanticSimilarity: result.relevanceScore > 50 ? 0.9 : 
                           ...
                           result.relevanceScore > 10 ? 0.5 : 0.3,
...
function generatePrerequisites(result: any, query: string): string[] {
...
function calculateQueryComplexity(query: string): 'basic' | 'intermediate' | 'advanced' {
  const advancedKeywords = ['transformer', 'attention', 'gradient', 'optimization', 'regularization'];
...
function analyzeTopicCoverage(query: string): string[] {
  ...
  const topics = ['machine learning', 'deep learning', 'natural language processing', 'computer vision', 'reinforcement learning'];
  const lowerQuery = query.toLowerCase();
