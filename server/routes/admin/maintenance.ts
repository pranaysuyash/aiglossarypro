...
export function registerAdminMaintenanceRoutes(app: Express): void {
  ...
  app.get('/api/admin/maintenance/status', authMiddleware, tokenMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Error fetching maintenance status", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      res.status(500).json({
        success: false,
  ...
  app.post('/api/admin/maintenance/clear-all', authMiddleware, tokenMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Error clearing all data", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      res.status(500).json({
        success: false,
  ...
  app.post('/api/admin/maintenance/backup', authMiddleware, tokenMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Error creating backup", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      res.status(500).json({
        success: false,
