...
router.get('/stats', (req, res) => {
  const stats = {
    ...
    combined: {
      totalHits: queryCache.getStats().hitCount + searchCache.getStats().hitCount + userCache.getStats().hitCount,
      totalMisses: queryCache.getStats().missCount + searchCache.getStats().missCount + userCache.getStats().missCount,
      totalSize: queryCache.getStats().size + searchCache.getStats().size + userCache.getStats().size,
      overallHitRate: 0
    }
  };
  ...
  try {
    ...
    switch (cacheType) {
      ...
      case 'user':
        ...
        break;
      case 'all':
        clearedCount = queryCache.getStats().size + searchCache.getStats().size + userCache.getStats().size;
        queryCache.invalidateAll();
        searchCache.invalidateAll();
        userCache.invalidateAll();
        break;
...
function analyzeTrends(metrics: any[]): any {
  ...
  return {
    ...
    responseTimeTrend: calculateAverage(secondHalf, 'avgResponseTime') < calculateAverage(firstHalf, 'avgResponseTime') ? 'improving' : 'degrading'
  };
}
...
function generateRecommendations(metrics: any[], currentStats: any, health: any): string[] {
  ...
  if (avgHitRate < 0.5) {
    recommendations.push('Very low hit rate detected. Review cache key generation and TTL settings');
  }
  
  const totalSize = currentStats.query.size + currentStats.search.size + currentStats.user.size;
  const totalMax = currentStats.query.maxItems + currentStats.search.maxItems + currentStats.user.maxItems;
