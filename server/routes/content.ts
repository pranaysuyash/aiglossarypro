...
export function registerContentRoutes(app: Express): void {
  ...
  app.post('/api/content/accessibility/batch-analyze', multiAuthMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.put('/api/content/accessibility/term/:id/improve', multiAuthMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
      const termId = req.params.id;
  ...
  app.get('/api/content/accessibility/report', multiAuthMiddleware, requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/content/accessibility/search', 
      ...
      try {
        ...
        if (needsImprovement !== undefined) {
          ...
          filteredTerms = filteredTerms.filter(term => 
            (term.accessibilityScore.score < 70) === needsFilter
          );
        }
