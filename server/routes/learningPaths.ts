...
import { type ValidatedRequest, validateBody, validateQuery } from '../middleware/validation';
...
export function registerLearningPathsRoutes(app: Express): void {
    ...
    async (req: ValidatedRequest<any, LearningPathsQuery>, res: Response) => {
      try {
        const {
          category,
          difficulty,
          limit = LEARNING_PATHS_LIMITS.DEFAULT_LIMIT,
          offset = 0,
        } = req.validatedQuery || {};
        ...
        const conditions = [eq(learningPaths.is_published, true)];
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const [progress] = await db.insert(userLearningProgress).values(progressData).returning();
          ...
          metadata: {
            ...
            avgScore:
              recommendedPaths.length > 0
                ? Math.round(
                    recommendedPaths.reduce((sum, p) => sum + p.recommendationScore, 0) /
      ...
      try {
        ...
        if (existingPath[0].created_by !== user.id && !user.isAdmin) {
          return res.status(403).json({
            success: false,
            message:
              'Permission denied. Only the creator or admin can add steps to this learning path.',
          });
        }
        ...
        if (!finalStepOrder) {
          const maxOrderResult = await db
            .select({ maxOrder: sql<number>`MAX(${learningPathSteps.step_order})` })
            .from(learningPathSteps)
            .where(eq(learningPathSteps.learning_path_id, pathId));
        ...
        const [createdStep] = await db.insert(learningPathSteps).values(stepData).returning();
   ...
   */
  app.put(
    '/api/learning-paths/:pathId/steps/:stepId',
    multiAuthMiddleware,
    async (req: Request, res: Response) => {
      try {
        const user = (req as any).user;
        ...
        if (existingPath[0].created_by !== user.id && !user.isAdmin) {
          return res.status(403).json({
            success: false,
            message:
              'Permission denied. Only the creator or admin can update steps in this learning path.',
          });
        }
        ...
        const existingStep = await db
          ...
          .from(learningPathSteps)
          .where(
            and(eq(learningPathSteps.id, stepId), eq(learningPathSteps.learning_path_id, pathId))
          )
          .limit(1);
      ...
      try {
        ...
        if (existingPath[0].created_by !== user.id && !user.isAdmin) {
          return res.status(403).json({
            success: false,
            message:
              'Permission denied. Only the creator or admin can delete steps from this learning path.',
          });
        }
   ...
   */
  app.patch(
    '/api/learning-paths/:pathId/steps/reorder',
    multiAuthMiddleware,
    async (req: Request, res: Response) => {
      try {
        const user = (req as any).user;
        ...
        if (existingPath[0].created_by !== user.id && !user.isAdmin) {
          return res.status(403).json({
            success: false,
            message:
              'Permission denied. Only the creator or admin can reorder steps in this learning path.',
          });
        }
