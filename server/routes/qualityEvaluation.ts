...
const _qualityAnalyticsSchema = z.object({
 ...
 * @access Private
 */
router.get('/trends/:termId', authenticateToken, async (req, res, next) => {
  try {
    const trendAnalysis = await qualityAnalyticsService.getQualityTrendAnalysis(req.params.termId);
 ...
 * @access Private
 */
router.get('/segments', authenticateToken, async (req, res, next) => {
  try {
    const { segmentType = 'category', startDate, endDate } = req.query;
 ...
 * @access Private
 */
router.get('/recommendations/:termId?', authenticateToken, async (req, res, next) => {
  try {
...
router.get('/export', requireAdmin, async (req, res, next) => {
  try {
    ...
    const exportData = await qualityAnalyticsService.exportQualityData(format as any, {
 ...
 * @access Admin
 */
router.get('/metrics/realtime', requireAdmin, async (_req, res, next) => {
  try {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const auditConfig = await qualityAnalyticsService.scheduleQualityAudit(req.body.schedule, {
 ...
 * @access Admin
 */
router.get('/flagged', requireAdmin, async (req, res, next) => {
  try {
 ...
 * @access Private
 */
router.get('/improvements/:termId', authenticateToken, async (req, res, next) => {
  try {
