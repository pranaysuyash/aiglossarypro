...
export function registerSubcategoryRoutes(app: Express): void {
  ...
  app.get('/api/subcategories/:id', async (req: Request, res: Response) => {
    try {
      ...
      log.error(`Error fetching subcategory ${req.params.id}`, { error, component: 'SubcategoryRoutes', subcategoryId: req.params.id });
      res.status(500).json({ 
        success: false,
  ...
  app.get('/api/subcategories/:id/terms', async (req: Request, res: Response) => {
    try {
      ...
      log.error(`Error fetching terms for subcategory ${req.params.id}`, { error, component: 'SubcategoryRoutes', subcategoryId: req.params.id });
      res.status(500).json({ 
        success: false,
  ...
  app.get('/api/categories/:categoryId/subcategories', async (req: Request, res: Response) => {
    try {
      ...
      log.error(`Error fetching subcategories for category ${req.params.categoryId}`, { error, component: 'SubcategoryRoutes', categoryId: req.params.categoryId });
      res.status(500).json({ 
        success: false,
