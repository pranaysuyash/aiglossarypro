...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const fileName = addTimestamp === 'true' 
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const tempFilePath = path.join(tempDir, `bulk_upload_${Date.now()}_${i}_${file.originalname}`);
...
router.get('/download/:key(*)', async (req, res) => {
  try {
    ...
    const shouldDecompress = decompress === 'true' || 
      (decompress === 'auto' && key.endsWith('.gz'));
...
router.post('/archive', async (req, res) => {
  try {
    ...
    const s3Client = getOptimizedS3Client();
    const archivePath = await s3Client.createArchive(
      keys,
      archiveName,
      format as 'zip' | 'tar'
    );
