...
async function seed() {
  ...
  const aiCategory = await db.insert(categories).values({
    ...
    description: 'The field of computer science dedicated to creating systems that can perform tasks requiring human intelligence.'
  }).returning();
  
  const mlCategory = await db.insert(categories).values({
    name: 'Machine Learning',
    description: 'A subset of AI focused on systems that can learn from data without explicit programming.'
  }).returning();
  
  const dlCategory = await db.insert(categories).values({
    name: 'Deep Learning',
  ...
  const aiSubcategories = await db.insert(subcategories).values([
  ...
  const mlSubcategories = await db.insert(subcategories).values([
  ...
  const dlSubcategories = await db.insert(subcategories).values([
  ...
  console.log('Subcategories created:', 
    aiSubcategories.length + 
    mlSubcategories.length + 
    dlSubcategories.length
  );
  
  // Insert terms
  const aiTerms = await db.insert(terms).values([
    {
      name: 'Artificial Intelligence',
      shortDefinition: 'Systems designed to mimic human intelligence',
      definition: 'Artificial Intelligence (AI) refers to systems or machines that mimic human intelligence to perform tasks and can iteratively improve themselves based on the information they collect.',
    ...
    },
    {
      ...
      definition: 'The Turing Test, proposed by Alan Turing in 1950, is a test of a machine\'s ability to exhibit intelligent behavior equivalent to, or indistinguishable from, that of a human.',
      categoryId: aiCategory[0].id,
      characteristics: ['Human-like interaction', 'Natural language processing', 'Reasoning']
    }
  ]).returning();
  
  const mlTerms = await db.insert(terms).values([
    {
      name: 'Supervised Learning',
      shortDefinition: 'Training with labeled data',
      definition: 'Supervised learning is a type of machine learning where the algorithm learns from labeled training data, helping it to predict outcomes for unforeseen data.',
    ...
    },
    {
      ...
      definition: 'Unsupervised learning is a type of machine learning where the algorithm learns patterns from unlabeled data without explicit instructions on what to look for.',
      categoryId: mlCategory[0].id,
      characteristics: ['Unlabeled data', 'Pattern recognition', 'Clustering', 'Dimensionality reduction']
    }
  ]).returning();
  
  const dlTerms = await db.insert(terms).values([
    {
      name: 'Neural Network',
      shortDefinition: 'Computing systems inspired by biological neural networks',
      definition: 'Neural networks are computing systems inspired by the biological neural networks that constitute animal brains. They are capable of machine learning as well as pattern recognition.',
    ...
    },
    {
      ...
      definition: 'A Convolutional Neural Network (CNN) is a class of deep neural networks most commonly applied to analyzing visual imagery. They are designed to automatically and adaptively learn spatial hierarchies of features from input images.',
      categoryId: dlCategory[0].id,
