...
import { eq, and, desc, sql, inArray, not, isNull, gte } from 'drizzle-orm';
...
export class AIRecommendationService {
   ...
   */
  async getPersonalizedRecommendations(userId: string, limit: number = 10): Promise<RecommendedPath[]> {
    try {
      // Build user profile
      ...
      const recommendations = scoredPaths
  ...
  private async buildUserProfile(userId: string): Promise<UserProfile> {
    const [
      ...
      // Completed paths
      db.select({ path_id: userLearningProgress.learning_path_id })
      ...
      // Favorite terms
      db.select({ term_id: favorites.term_id })
        .from(favorites)
    ...
    const avgSessionTime = currentPathsResult.length > 0 
      ? currentPathsResult.reduce((sum, p) => sum + (Number(p.time_spent) || 0), 0) / currentPathsResult.length
      : 0;
    ...
    const difficultyPreference = this.determineDifficultyPreference(skillLevel, completedPathsResult);
   ...
   */
  private determineDifficultyPreference(skillLevel: number, completedPaths: any[]): 'beginner' | 'intermediate' | 'advanced' {
  ...
  private async getCandidatePaths(userProfile: UserProfile) {
    ...
    let whereConditions = [eq(learningPaths.is_published, true)];
   ...
   */
  private scoreDifficulty(pathDifficulty: string, userProfile: UserProfile): { score: number; reason?: string } {
    ...
    if (pathLevel === userPref) {
      return { score: 25, reason: `Perfect difficulty match for your ${userProfile.difficultyPreference} level` };
    } else if (Math.abs(pathLevel - userPref) === 1) {
      return { score: 15, reason: `Slightly ${pathLevel > userPref ? 'challenging' : 'easier'} than your usual level` };
    } else {
      return { score: 5 };
    }
  }
   ...
   */
  private async scorePrerequisites(path: any, userProfile: UserProfile): Promise<{ score: number; reason?: string }> {
   ...
   */
  private scoreDuration(duration: number, userProfile: UserProfile): { score: number; reason?: string } {
   ...
   */
  private async scoreContentRelevance(path: any, userProfile: UserProfile): Promise<{ score: number; reason?: string }> {
    if (userProfile.favoriteTerms.length === 0) {
      return { score: 5 }; // Neutral score if no favorites
    }
    
    // Get terms used in this learning path
    const pathTerms = await db.select({ term_id: learningPathSteps.term_id })
    ...
    const overlap = pathTermIds.filter(termId => 
      userProfile.favoriteTerms.includes(termId)
    );
   ...
   */
  private async enrichRecommendations(scoredPaths: RecommendationScore[]): Promise<RecommendedPath[]> {
    if (scoredPaths.length === 0) return [];
    
    const pathIds = scoredPaths.map(p => p.pathId);
    
    const pathsData = await db.select()
      .from(learningPaths)
  ...
  async getTrendingPaths(limit: number = 5): Promise<RecommendedPath[]> {
    ...
    const trendingPaths = await db.select({
      id: learningPaths.id,
      name: learningPaths.name,
      description: learningPaths.description,
      difficulty_level: learningPaths.difficulty_level,
