...
export class CDNMonitoringService {
  ...
  private async collectCloudflareMetrics(): Promise<CDNMetrics> {
    try {
      ...
      return {
        ...
        errorRate: (analytics.totals.requests?.http_status_4xx + analytics.totals.requests?.http_status_5xx) / 
  ...
  private async collectLocalMetrics(): Promise<CDNMetrics> {
    ...
    const successfulChecks = healthChecks.filter(check => check.status >= 200 && check.status < 400);
  ...
  private determineHealthStatus(analytics: any): 'healthy' | 'degraded' | 'down' {
    ...
    const errorRate = (analytics.totals.requests?.http_status_4xx + analytics.totals.requests?.http_status_5xx) / 
                     (analytics.totals.requests?.all || 1);
  ...
  private async sendToExternalServices(message: string, metrics: CDNMetrics): Promise<void> {
  ...
  public monitoringMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
      ...
      if (process.env.CDN_VERBOSE_LOGGING === 'true') {
        console.log(`[CDN Monitor] ${req.method} ${req.url} - ${res.statusCode} - ${responseTime}ms`);
      }
  ...
  private trackRequest(req: Request, res: Response, responseTime: number): void {
