...
export class SupportService {
   ...
   */
  async getTicketStats(filters: { dateFrom?: Date } = {}) {
    try {
      const { dateFrom } = filters;
      
      // Base query condition
      const whereCondition = dateFrom ? gte(supportTickets.createdAt, dateFrom) : undefined;
      ...
      return {
        ...
        priorityCounts: priorityCounts.reduce((acc, { priority, count }) => {
          acc[priority] = count;
          return acc;
        }, {} as Record<string, number>),
   ...
   */
  async assignTicket(ticketId: string, assignToUserId: string, assignedBy: string) {
    try {
      await db
        .update(supportTickets)
        .set({
      ...
      // Log the assignment
      await db.insert(supportTicketHistory).values({
        ticketId,
        action: 'assigned',
      ...
      log.info('Ticket assigned', { ticketId, assignToUserId, assignedBy });
    } catch (error) {
      log.error('Error assigning ticket', { error, ticketId, assignToUserId });
      throw error;
    }
  }

  /**
   * Get ticket messages with optional internal notes
   */
  async getTicketMessages(ticketId: string, includeInternalNotes: boolean = false) {
    try {
      const whereCondition = includeInternalNotes 
        ? eq(supportMessages.ticketId, ticketId)
        : and(eq(supportMessages.ticketId, ticketId), eq(supportMessages.isInternalNote, false));
      ...
      log.error('Error getting ticket messages', { error, ticketId });
      throw error;
    }
  }
}
