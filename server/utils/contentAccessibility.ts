...
const COMPLEX_TERMS: Record<string, { simple: string; explanation: string }> = {
  'hyperparameter': { 
    ...
    explanation: 'A setting that controls how a machine learning model learns, like adjusting the speed of learning'
  },
  'backpropagation': { 
    simple: 'error correction method', 
    explanation: 'A technique where neural networks learn by working backwards from mistakes to improve their performance'
  },
  'overfitting': { 
    simple: 'memorizing too specifically', 
    explanation: 'When a model learns the training data so well that it cannot handle new, unseen data effectively'
  },
  'gradient descent': { 
    simple: 'optimization method', 
    explanation: 'A mathematical technique to find the best solution by gradually improving guesses, like rolling a ball downhill to find the bottom'
  },
  'convolutional neural network': { 
    simple: 'image recognition AI', 
    explanation: 'A type of artificial intelligence specifically designed to understand and analyze images by detecting patterns and features'
  },
  'natural language processing': { 
    simple: 'language understanding AI', 
    explanation: 'Technology that helps computers understand, interpret, and work with human language, like text and speech'
  },
  'reinforcement learning': { 
    simple: 'learning through trial and error', 
    explanation: 'A learning method where AI improves by trying actions and getting rewards or penalties, similar to how we learn from experience'
  },
  'transformer architecture': { 
    simple: 'attention-based AI model', 
    explanation: 'A powerful AI design that processes information by paying attention to relevant parts, enabling better understanding of context'
  },
  'unsupervised learning': { 
    simple: 'pattern discovery without examples', 
    explanation: 'A learning approach where AI finds hidden patterns in data without being given specific examples of what to look for'
  },
  'dimensionality reduction': { 
    simple: 'data simplification', 
    explanation: 'A technique to reduce complex data to simpler forms while keeping the most important information'
  }
};
...
export function calculateAccessibilityScore(content: string): AccessibilityScore {
  ...
  if (averageWordsPerSentence <= READING_LEVELS.ELEMENTARY.maxWordsPerSentence && 
...
export function addBeginnerContext(definition: string, termName: string): string {
  ...
  const introductions = {
    ...
    'machine learning': 'This is a way for computers to learn and improve from experience, just like humans do.',
    'algorithm': 'An algorithm is like a recipe - a set of step-by-step instructions to solve a problem.',
    'artificial intelligence': 'AI is technology that enables machines to think and make decisions like humans.',
  ...
  if (!definition.toLowerCase().includes('example') && 
      !definition.toLowerCase().includes('for instance')) {
    const examples = {
      'supervised learning': 'For example, teaching a computer to recognize cats by showing it thousands of labeled cat photos.',
      'clustering': 'For instance, grouping customers by their shopping habits to create targeted marketing campaigns.',
    ...
    const matchingExample = Object.entries(examples).find(([key, _]) => 
      termName.toLowerCase().includes(key) || definition.toLowerCase().includes(key)
    );
 ...
 */
export async function processTermsForAccessibility(terms: Array<{ id: string; name: string; definition: string }>) {
  const results = [];
  
  for (const term of terms) {
    try {
      const score = calculateAccessibilityScore(term.definition);
      const suggestion = score.level !== 'beginner' ? 
        generateSimplificationSuggestion(term.definition) : null;
 ...
 */
export function generateAccessibilityReport(terms: Array<{ score: AccessibilityScore; termName: string }>) {
