...
const languageSchema = z.enum([
  ...
  'csharp', 'go', 'rust', 'r', 'matlab', 'sql', 'html', 'css'
]);
const exampleTypeSchema = z.enum([
  'implementation', 'tutorial', 'snippet', 'project', 
  'visualization', 'algorithm', 'demo', 'exercise'
]);
...
export const createCodeExampleSchema = z.object({
  ...
  code: z.string()
    ...
    .max(50000, 'Code must be 50,000 characters or less'),
  expected_output: z.string()
    .max(10000, 'Expected output must be 10,000 characters or less')
    .optional(),
  explanation: z.string()
    .max(5000, 'Explanation must be 5,000 characters or less')
    .optional(),
  libraries: z.array(z.string().trim().min(1))
...
export const runCodeSchema = z.object({
  input_data: z.string()
    .max(10000, 'Input data must be 10,000 characters or less')
    .optional(),
  environment: z.string()
    .max(100, 'Environment must be 100 characters or less')
    .optional()
});
