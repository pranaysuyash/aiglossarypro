...
export const useEnhanced295Content = (termId: string, termName?: string) => {
  ...
  // Fetch hierarchical content for the term
  const { 
    data: hierarchicalContent, 
    isLoading: contentLoading,
    refetch: refetchContent
  } = useQuery({
    queryKey: ['term-hierarchical-content', termId],
    queryFn: async () => {
      const response = await api.get(`/api/enhanced-295/term/${termId}/hierarchical-content`);
      return response.data.hierarchicalContent;
    },
    enabled: !!termId,
  });
  ...
  return {
    ...
    // Loading states
    isLoading: structureLoading || statusLoading || contentLoading,
    structureLoading,
    statusLoading,
    contentLoading,
