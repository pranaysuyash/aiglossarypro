  ...
  test.describe('Basic Search', () => {
    test('should perform basic keyword search', async ({ page }) => {
      ...
      const searchInput = page
        .locator(
          '[data-testid="search-input"], input[type="text"][placeholder*="search" i], input[role="combobox"]'
        )
        .first();
      ...
      await expect(page).toHaveURL(
        /.*\/terms\?.*search.*machine.*learning|.*\/search\?.*machine.*learning/
      );
      ...
      const searchResults = page.locator(
        '[data-testid="term-card"], [data-testid="search-result"], .search-result'
      );
      await expect(searchResults.first()).toBeVisible({ timeout: 10000 });

      // Verify search query is reflected in the page
      const searchQuery = page.locator(
        '[data-testid="search-query"], input[value*="machine learning"]'
      );
    ...
    test('should display search suggestions while typing', async ({ page }) => {
      ...
      const suggestions = page.locator(
        '[data-testid="search-suggestions"], [role="listbox"], .suggestions-dropdown'
      );
      ...
      if ((await suggestions.count()) > 0) {
        ...
        const suggestionItems = suggestions.locator(
          '[data-testid="suggestion-item"], [role="option"], li'
        );
    ...
    test('should handle special characters in search', async ({ page }) => {
      ...
      const specialQueries = ['C++', 'AI/ML', 'back-propagation', 'α-β pruning', 'F1-score'];
  ...
  test.describe('Advanced Search', () => {
    test('should use advanced search filters', async ({ page }) => {
      ...
      const difficultyFilter = page.locator(
        '[data-testid="difficulty-filter"], select[name="difficulty"]'
      );
      ...
      const searchButton = page.locator(
        '[data-testid="search-submit"], button[type="submit"]:has-text("Search")'
      );
    ...
    test('should apply and remove search filters', async ({ page }) => {
      ...
      for (const control of filterControls) {
        if ((await control.count()) > 0) {
          ...
          const categoryFilter = control.locator(
            '[data-testid="category-filter"], select, [role="combobox"]'
          );
          const difficultyFilter = control.locator(
            '[data-testid="difficulty-filter"], input[type="checkbox"]'
          );
      ...
      for (const control of sortControls) {
        if ((await control.count()) > 0) {
          ...
          const initialResults = await page
            .locator('[data-testid="term-card"] h3, [data-testid="term-card"] h2')
            .allTextContents();
