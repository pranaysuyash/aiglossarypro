...
describe('Country-Based Pricing (PPP) Testing', () => {
  ...
  describe('Country Detection', () => {
    ...
    test('should apply PPP discount for developing countries', async () => {
      ...
      for (const testCase of testCases) {
        ...
        console.log(`${testCase.country}: $${testCase.expectedPrice} (${testCase.expectedFactor * 100}% of base price)`);
      }
    });
  ...
  describe('Gumroad Integration with PPP', () => {
    test('should pass correct pricing to Gumroad checkout', async () => {
      ...
      expect(window.GumroadOverlay.open).toHaveBeenCalledWith(
        expect.stringContaining('price=174')
      );
    });
    ...
    test('should support A/B testing different discount levels', async () => {
      ...
      for (const variant of variants) {
        const mockResponse = {
          ...
          variant: variant,
          pricing: {
            lifetime: variant === 'high_discount' ? 79 : 
                     variant === 'medium_discount' ? 99 : 
                     124, // control
            currency: 'USD',
            symbol: '$',
            pppFactor: variant === 'high_discount' ? 0.32 : 
                      variant === 'medium_discount' ? 0.4 : 
                      0.5
          }
        };
