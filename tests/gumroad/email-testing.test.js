...
import { sendPremiumWelcomeEmail } from '../../server/utils/email';
import { getPremiumWelcomeEmailTemplate, } from '../../server/utils/emailTemplates';
// Mock nodemailer
vi.mock('nodemailer');
describe('Premium Welcome Email Testing', () => {
    ...
    beforeEach(() => {
        ...
        nodemailer.createTransporter = vi.fn().mockReturnValue(mockTransporter);
        vi.clearAllMocks();
    });
        ...
        test('should validate email addresses', async () => {
            ...
            for (const invalidEmail of invalidEmails) {
                ...
                try {
                    ...
                    expect(mockTransporter.sendMail).not.toHaveBeenCalledWith(expect.objectContaining({
                        to: [invalidEmail],
                    }));
                }
                catch (error) {
