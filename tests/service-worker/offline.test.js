...
describe('Service Worker', () => {
    ...
    describe('Activate Event', () => {
        it('should clean up old caches on activate', async () => {
            ...
            const activatePromise = mockCaches.keys().then((cacheNames) => {
                const deletePromises = cacheNames
                    .filter(cacheName => !cacheName.includes('v1.2.0') && // Keep current version
    ...
    describe('Push Notifications', () => {
        it('should show notification on push event', async () => {
            ...
            const pushPromise = mockServiceWorker.registration.showNotification('New Content', notificationOptions);
            pushEvent.waitUntil(pushPromise);
            await pushPromise;
            expect(mockServiceWorker.registration.showNotification).toHaveBeenCalledWith('New Content', notificationOptions);
    ...
    describe('Cache Management', () => {
        it('should provide cache size information', async () => {
            ...
            const cacheInfo = await mockCache.keys().then((keys) => ({
        ...
        it('should clear all caches when requested', async () => {
            ...
            const clearResult = await mockCaches
                .keys()
                .then((names) => Promise.all(names.map(name => mockCaches.delete(name))));
