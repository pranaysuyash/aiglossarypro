...
describe('AISemanticSearch', () => {
    ...
    it('renders search input correctly', () => {
        render(_jsx(AISemanticSearch, {}), { wrapper: createWrapper() });
    ...
    it('performs semantic search when user types', async () => {
        ...
        await waitFor(() => {
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/ai/semantic-search'), expect.objectContaining({
                method: 'POST',
    ...
    it('calls onResultClick when result is clicked', async () => {
        ...
        const user = userEvent.setup();
        render(_jsx(AISemanticSearch, { onResultClick: mockOnResultClick }), { wrapper: createWrapper() });
        const searchInput = screen.getByPlaceholderText(/search/i);
        await user.type(searchInput, 'neural');
        await waitFor(() => {
            expect(screen.getByText('Neural Networks')).toBeInTheDocument();
