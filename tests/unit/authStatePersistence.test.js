...
describe('AuthStatePersistence', () => {
    ...
    describe('storeAuthState', () => {
        it('should store authentication state successfully', async () => {
            ...
            expect(mockSessionStorage.setItem).toHaveBeenCalledWith('aiglossary_auth_state', expect.any(String));
            expect(mockSessionStorage.setItem).toHaveBeenCalledWith('aiglossary_auth_state_user', JSON.stringify(mockUser));
            expect(mockLocalStorage.setItem).toHaveBeenCalledWith('aiglossary_auth_state_user', JSON.stringify(mockUser));
        });
    ...
    describe('refreshTokenIfNeeded', () => {
        ...
        it('should prevent concurrent refresh attempts', async () => {
            ...
            const { refreshIdToken } = await import('../../client/src/lib/firebase');
            vi.mocked(refreshIdToken).mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('new-token'), 1000)));
    ...
    describe('recoverFromNetworkError', () => {
        ...
        it('should fallback to Firebase user when stored state is invalid', async () => {
            ...
            vi.mocked(getCurrentUser).mockReturnValue(mockFirebaseUser);
