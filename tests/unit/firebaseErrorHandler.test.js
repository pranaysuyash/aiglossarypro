...
import { FirebaseErrorHandler } from '../../client/src/lib/FirebaseErrorHandler';
...
describe('FirebaseErrorHandler', () => {
    ...
    describe('Error Handling', () => {
        it('should create Firebase error with correct properties', async () => {
            ...
            const firebaseError = await errorHandler.handleAuthError(originalError, 'signIn', { email: 'test@example.com' });
        ...
        it('should handle timeout errors', async () => {
            ...
            const firebaseError = await errorHandler.handleAuthError(timeoutError, 'signIn');
            expect(firebaseError.type).toBe(ErrorType.AUTHENTICATION);
        ...
        it('should set appropriate severity levels', async () => {
            ...
            const firebaseError = await errorHandler.handleAuthError(criticalError, 'signIn');
    ...
    describe('Retry Logic', () => {
        it('should retry operations with exponential backoff', async () => {
            ...
            const result = await errorHandler.retryOperation(mockOperation, 'testOperation', { maxRetries: 3, baseDelay: 100 });
            expect(result).toBe('success');
            expect(mockOperation).toHaveBeenCalledTimes(3);
        });
        it('should fail after max retries', async () => {
            const mockOperation = vi.fn().mockRejectedValue(new Error('Persistent error'));
            await expect(errorHandler.retryOperation(mockOperation, 'testOperation', { maxRetries: 2, baseDelay: 100 })).rejects.toThrow('Persistent error');
        ...
        it('should not retry non-retryable errors', async () => {
            ...
            await expect(errorHandler.retryOperation(mockOperation, 'testOperation')).rejects.toThrow('User not found');
    ...
    describe('Circuit Breaker', () => {
        it('should open circuit breaker after repeated failures', async () => {
            ...
            for (let i = 0; i < 6; i++) {
                await errorHandler.handleAuthError(new Error('Test error'), 'testOperation');
            }
            // Circuit breaker should be open, making operations non-retryable
            const error = await errorHandler.handleAuthError(new Error('Another error'), 'testOperation');
    ...
    describe('User Messages', () => {
        it('should provide user-friendly error messages', async () => {
            ...
            for (const testCase of testCases) {
                ...
                const error = await errorHandler.handleAuthError(firebaseError, 'testOperation');
        ...
        it('should provide default message for unknown errors', async () => {
            ...
            const error = await errorHandler.handleAuthError(unknownError, 'testOperation');
