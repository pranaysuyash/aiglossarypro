...
import { createTimeoutWrapper, DEFAULT_TIMEOUTS, raceWithTimeout, TimeoutError, TimeoutQueue, withBatchTimeout, withTimeout, } from '../../client/src/lib/FirebaseTimeoutWrapper';
describe('FirebaseTimeoutWrapper', () => {
    ...
    describe('withTimeout', () => {
        ...
        it('should reject with TimeoutError when operation exceeds timeout', async () => {
            const mockOperation = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 2000)));
            const config = {
                timeout: 1000,
                operation: 'test',
                retryOnTimeout: false,
    ...
    describe('withBatchTimeout', () => {
        ...
        it('should timeout if global timeout is exceeded', async () => {
            const operations = [
                {
                    operation: vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 3000))),
                    name: 'slowOp',
                },
            ];
    ...
    describe('raceWithTimeout', () => {
        it('should return result from first successful operation', async () => {
            const operations = [
                {
                    operation: vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('slow'), 1000))),
                    name: 'slowOp',
                    priority: 2,
                },
                {
        ...
        it('should timeout if no operation completes in time', async () => {
            const operations = [
                {
                    operation: vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 2000))),
                    name: 'slowOp',
                    priority: 1,
                },
            ];
    ...
    describe('TimeoutQueue', () => {
        ...
        it('should queue operations beyond concurrency limit', async () => {
            let resolveOp1;
            let resolveOp2;
            const mockOp1 = vi.fn().mockImplementation(() => new Promise(resolve => { resolveOp1 = resolve; }));
            const mockOp2 = vi.fn().mockImplementation(() => new Promise(resolve => { resolveOp2 = resolve; }));
