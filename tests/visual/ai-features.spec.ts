...
test.describe('AI Features Visual Tests', () => {
  test('AI Definition Generator visual', async ({ page }) => {
    ...
    const aiGenerator = page.locator('[class*="ai-generator"], [data-testid*="ai-generator"], [class*="definition-generator"]').first();
    ...
    if (await aiGenerator.count() > 0) {
      ...
      await expect(page.locator('main, #root').first()).toHaveScreenshot('ai-definition-generator.png');
    }
  });
  ...
  test('AI Semantic Search visual', async ({ page }) => {
    ...
    const semanticSearch = page.locator('[class*="semantic-search"], [data-testid*="semantic-search"]').first();
  ...
  test('AI Content Feedback visual', async ({ page }) => {
    ...
    const feedback = page.locator('[class*="feedback"], [data-testid*="feedback"], button[class*="feedback"]').first();
  ...
  test('AI Term Suggestions visual', async ({ page }) => {
    ...
    const suggestions = page.locator('[class*="suggestions"], [data-testid*="suggestions"], [class*="recommended"]').first();
