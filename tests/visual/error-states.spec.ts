...
test.describe('Error States Visual Tests', () => {
  ...
  test('Error boundary visual', async ({ page }) => {
    ...
    const errorBoundary = page.locator('[class*="error-boundary"], [data-testid*="error"], .error').first();
  ...
  test('Empty states visual', async ({ page }) => {
    ...
    const emptyState = page.locator('[class*="empty"], [class*="no-results"], [data-testid*="empty"]').first();
