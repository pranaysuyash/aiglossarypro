...
const _WCAG_CRITERIA = {
    ...
    operable: {
        ...
        '2.2': {
            name: 'Enough Time',
            criteria: ['2.2.1 Timing Adjustable (A)', '2.2.2 Pause, Stop, Hide (A)'],
        },
        '2.3': {
            name: 'Seizures and Physical Reactions',
            criteria: ['2.3.1 Three Flashes or Below Threshold (A)'],
        },
        '2.4': {
            name: 'Navigable',
            criteria: [
    ...
    robust: {
        '4.1': {
            name: 'Compatible',
            criteria: ['4.1.1 Parsing (A)', '4.1.2 Name, Role, Value (A)', '4.1.3 Status Messages (AA)'],
        },
    },
};
class WCAGAccessibilityAuditor {
     ...
     */
    auditKeyboardNavigation(_component) {
        const results = [];
        // 2.1.1 Keyboard
        results.push({
     ...
     */
    auditColorAndContrast(_component) {
        const results = [];
     ...
     */
    auditTextAlternatives(_component) {
        const results = [];
     ...
     */
    auditResponsiveDesign(_component) {
        const results = [];
        // 1.4.4 Resize text
        results.push({
     ...
     */
    auditComponent(component, location) {
        ...
        const earnedPoints = allResults.reduce((sum, result) => {
            switch (result.status) {
                ...
                case 'fail':
                    return sum + 0;
                case 'not-applicable':
                    return sum + 100; // Don't penalize N/A
                default:
                    return sum;
            }
        }, 0);
    ...
    async runCompleteAudit() {
        ...
        this.overallScore = Math.round(this.results.reduce((sum, result) => sum + result.overallScore, 0) / this.results.length);
        return this.results;
    }
    /**
    ...
    generateReport() {
        ...
        report.push('');
        // Summary statistics
        const totalIssues = this.results.reduce((sum, result) => sum + result.results.filter(r => r.status === 'fail' || r.status === 'partial').length, 0);
        const criticalIssues = this.results.reduce((sum, result) => sum + result.results.filter(r => r.impact === 'critical' && r.status !== 'pass').length, 0);
        const highIssues = this.results.reduce((sum, result) => sum + result.results.filter(r => r.impact === 'high' && r.status !== 'pass').length, 0);
        ...
        for (const result of this.results) {
            const criticalCount = result.results.filter(r => r.impact === 'critical' && r.status !== 'pass').length;
        ...
        priorityIssues.sort((a, b) => {
            if (a.impact === 'critical' && b.impact !== 'critical')
                return -1;
            if (b.impact === 'critical' && a.impact !== 'critical')
                return 1;
            return 0;
        });
        let issueNumber = 1;
    ...
    saveResults() {
        ...
        const jsonData = {
            ...
            summary: {
                ...
                criticalIssues: this.results.reduce((sum, result) => sum + result.results.filter(r => r.impact === 'critical' && r.status !== 'pass').length, 0),
                highPriorityIssues: this.results.reduce((sum, result) => sum + result.results.filter(r => r.impact === 'high' && r.status !== 'pass').length, 0),
            },
        };
        try {
            writeFileSync(join(outputDir, 'accessibility-audit-results.json'), JSON.stringify(jsonData, null, 2));
        ...
        try {
            writeFileSync(join(outputDir, 'accessibility-audit-report.md'), this.generateReport());
