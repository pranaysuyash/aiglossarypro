...
async function performComprehensiveInteractions(page, routeName) {
    const interactions = [];
...
async function analyzeWithAI(report) {
    ...
    try {
        ...
        await fs.writeFile(path.join(OUTDIR, 'analysis', 'ai-analysis.json'), JSON.stringify(analysis, null, 2));
...
function calculateOverallScore(report) {
    ...
    const successfulInteractions = report.routes.reduce((acc, route) => acc + route.interactions.filter(i => i.success).length, 0);
    return totalInteractions > 0 ? Math.round((successfulInteractions / totalInteractions) * 100) : 0;
}
function findCriticalIssues(report) {
    const issues = [];
...
function generateRecommendations(report) {
    ...
    const formsWithErrors = routesWithForms.filter(r => r.interactions.some(i => i.type === 'form' && !i.success));
    if (formsWithErrors.length > 0) {
        recommendations.push('Review form validation and error handling');
    }
    return recommendations;
}
function analyzeUIIssues(route) {
    const issues = [];
...
function analyzeUXIssues(route) {
    const issues = [];
...
function analyzeAccessibilityIssues(route) {
    const issues = [];
...
function analyzePerformanceIssues(route) {
    const issues = [];
...
function analyzeInteractionFlows(report) {
    const issues = [];
    report.routes.forEach(route => {
...
function analyzeFormUsability(report) {
    const issues = [];
    report.routes.forEach(route => {
...
async function generateHTMLReport(report, analysis) {
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                ${analysis.overall_assessment.critical_issues.map((issue) => `<div class="issue critical">${issue}</div>`).join('')}
                ${analysis.overall_assessment.critical_issues.length === 0 ? '<div class="issue">No critical issues found!</div>' : ''}
            </div>

            <div class="section">
                <h2>💡 Recommendations</h2>
                ${analysis.overall_assessment.recommendations.map((rec) => `<div class="issue">${rec}</div>`).join('')}
            </div>
