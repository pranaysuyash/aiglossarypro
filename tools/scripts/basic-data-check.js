...
async function basicDatabaseCheck() {
    ...
    `);
        sampleCategories.rows.forEach((cat, i) => {
    ...
    `);
        if (searchResults.rows.length > 0) {
            ...
            searchResults.rows.forEach((term) => {
    ...
    `);
        ...
        const completeness = (((quality.total_terms - quality.missing_definitions) / quality.total_terms) *
            100).toFixed(1);
