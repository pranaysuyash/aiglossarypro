...
async function mockAIService() {
    ...
    return {
        generateDefinition: async (termName, category, context) => {
        ...
        generateSectionContent: async (termName, sectionName) => {
...
async function bulkImportContent(config = DEFAULT_CONFIG) {
        ...
        for (let i = 0; i < termsToProcess.length; i += config.batchSize) {
            ...
            const batchPromises = batch.map(async ({ categoryName, term }) => {
                ...
                try {
                    ...
                    const definition = await aiService.generateDefinition(term.name, categoryName, `Priority: ${term.priority}, Complexity: ${term.complexity}, Focus: ${term.focusAreas?.join(', ')}`);
                    // Create the term
...
function printImportSummary(summary, config) {
...
async function saveImportReport(summary, config) {
    try {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (arg) {
            ...
            case '--quality-threshold':
                if (nextArg)
                    config.qualityThreshold = parseInt(nextArg, 10);
                break;
            case '--delay':
                if (nextArg)
                    config.delayBetweenBatches = parseInt(nextArg, 10);
                break;
            case '--category':
                if (nextArg)
                    config.categoryFilter = nextArg;
                break;
            case '--priority':
                if (nextArg && ['high', 'medium', 'low'].includes(nextArg)) {
                    config.priorityFilter = nextArg;
                }
                break;
            case '--sections':
                if (nextArg)
                    config.targetSections = nextArg.split(',').map(s => s.trim());
                break;
            case '--help':
            case '-h':
                console.log(`
