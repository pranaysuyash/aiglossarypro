...
async function checkTableStructure() {
  ...
  try {
    ...
    termsStructure.rows.forEach((col: any) => {
      console.log(
    ...
    enhancedStructure.rows.forEach((col: any) => {
      console.log(
        `${col.column_name}: ${col.data_type} ${col.is_nullable === 'YES' ? '(nullable)' : '(not null)'}`
      );
    });
    ...
    categoriesStructure.rows.forEach((col: any) => {
      console.log(
        `${col.column_name}: ${col.data_type} ${col.is_nullable === 'YES' ? '(nullable)' : '(not null)'}`
      );
    });
    ...
    sampleEnhanced.rows.forEach((term: any, i) => {
