...
async function checkBothSectionsTables() {
    try {
        ...
        if (basicSections.length > 0) {
            console.log('First 3:');
            basicSections.slice(0, 3).forEach(s => console.log(`- ${s.name} (Order: ${s.displayOrder})`));
        }
        ...
        if (contentSections.length > 0) {
            console.log('First 3:');
            contentSections.slice(0, 3).forEach(s => console.log(`- ${s.sectionName} (Type: ${s.displayType}, Has data: ${!!s.sectionData})`));
        }
    }
    catch (error) {
