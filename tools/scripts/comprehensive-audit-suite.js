...
class ComprehensiveAuditSuite {
    ...
    async runVisualAudit() {
        ...
        try {
            ...
            const comprehensiveResult = await this.executeCommand('tsx scripts/quick-visual-audit.ts', { timeout: 180000, allowFailure: true } // 3 minutes timeout for quick comprehensive testing
            );
            ...
            const storybookResult = await this.executeCommand('npx playwright test --config=playwright.config.js --reporter=json', { timeout: 300000, allowFailure: true } // 5 minutes timeout
            );
            // Page-level visual audit using existing foundation
            console.log('  🔍 Page-level visual regression testing...');
            const pageResult = await this.executeCommand('npx playwright test --config=playwright.visual.config.ts --reporter=json', { timeout: 300000, allowFailure: true });
            ...
            try {
                const reportPath = path.join(projectRoot, 'comprehensive-audit', 'comprehensive-report.json');
                if (fs.existsSync(reportPath)) {
                    ...
                    console.log(`    ✓ Loaded comprehensive audit results: ${comprehensiveResults.total} tests`);
                }
            }
            catch (error) {
                console.log(`    Warning: Could not read comprehensive audit JSON report: ${error.message || 'Unknown error'}`);
            }
            ...
            const totalPassed = comprehensiveResults.passed + componentResults.passed + pageResults.passed;
            const totalFailed = comprehensiveResults.failed + componentResults.failed + pageResults.failed;
            return {
                pillar: 'visual',
    ...
    async runAccessibilityAudit() {
        ...
        try {
            ...
            const result = await this.executeCommand('npx playwright test tests/e2e/accessibility.spec.ts --reporter=json', { timeout: 180000 });
    ...
    async runPerformanceAudit() {
        ...
        try {
            ...
            const reactScanResult = await this.executeCommand('npx react-scan@latest --build --report --output-file=reports/audit-suite/react-scan-report.json', { timeout: 120000, allowFailure: true });
            // Run Lighthouse performance audit
            console.log('    🔍 Lighthouse performance audit...');
            const lighthouseResult = await this.executeCommand('npx playwright test tests/e2e/performance.spec.ts --reporter=json', { timeout: 180000 });
        ...
        try {
            ...
            for (const testFile of e2eTests) {
                if (fs.existsSync(path.join(projectRoot, testFile))) {
                    const result = await this.executeCommand(`npx playwright test ${testFile} --reporter=json`, { timeout: 180000 });
    ...
    async runCodeQualityAudit() {
        ...
        try {
            ...
            await this.executeCommand(`npx eslint . --ext .js,.jsx,.ts,.tsx --format json --output-file ${eslintReportPath}`, { allowFailure: true });
            // Run Biome
            console.log('    🔍 Biome analysis...');
            const biomeResult = await this.executeCommand(`npx biome check . --reporter json --diagnostic-level=info`, { allowFailure: true });
    ...
    parseComprehensiveVisualResults(output) {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                if (totalMatch)
                    total = Math.max(total, parseInt(totalMatch[1] || totalMatch[2]));
            }
    ...
    parseEslintResults(eslintReport) {
    ...
    parseBiomeOutputText(biomeOutput) {
        ...
        const summaryLine = lines.find(line => line.includes('Found') || line.includes('issues') || line.includes('diagnostics'));
        if (summaryLine) {
    ...
    generateHtmlReport(timestamp, duration, overallStatus) {
...
<body>
        ...
        try {
            ...
            if (this.serverProcess.stdout) {
                this.serverProcess.stdout.on('data', data => {
                    ...
                    if (output.includes('Frontend server is ready') ||
    ...
    async checkServerRunning(url, timeout = 5000) {
        try {
            const result = await this.executeCommand(`curl --head --silent --fail --connect-timeout ${timeout / 1000} ${url}`, { timeout, allowFailure: true });
