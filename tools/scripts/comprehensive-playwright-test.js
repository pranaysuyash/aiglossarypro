...
class ComprehensivePlaywrightTest {
    ...
    results = [];
    screenshotCount = 0;
    constructor(baseUrl = 'http://localhost:5173', apiUrl = 'http://localhost:3001') {
    ...
    async runTest(name, testFn, user) {
    ...
    async handleBlockingModals(page) {
        try {
        ...
        for (const user of TEST_USERS) {
            await this.runTest(`Login - ${user.type} user`, async () => {
                ...
                if (user.type === 'admin') {
                    ...
                    if (!adminLink)
                        throw new Error('Admin dashboard link not visible');
                }
                await page.close();
                await context.close();
            }, user.email);
    ...
    async testTermDetailFeatures() {
        ...
        await this.runTest('Term sections and navigation', async () => {
            ...
            if (sections === 0)
                throw new Error('No term sections found');
    ...
    async testUserAccountFeatures() {
        ...
        await this.runTest('User profile page', async () => {
            ...
            if (profileElements === 0)
                throw new Error('Profile page not loading');
        ...
        await this.runTest('Bookmarks page', async () => {
            ...
            if (bookmarksTitle === 0)
                throw new Error('Bookmarks page not loading');
    ...
    async testDashboardComponents() {
        ...
        await this.runTest('Recent activity widget', async () => {
            ...
            if (recentActivity === 0)
                throw new Error('Recent activity widget not found');
        ...
        await this.runTest('Recommendations widget', async () => {
            ...
            if (recommendations === 0)
                throw new Error('Recommendations widget not found');
        ...
        await this.runTest('Learning path widget', async () => {
            ...
            if (learningPaths === 0)
                console.log(chalk.gray('   Learning path widget not found'));
            await page.close();
            await context.close();
        });
    }
    ...
    async testErrorHandling() {
        ...
        await this.runTest('404 page', async () => {
            ...
            if (notFoundElements === 0)
                throw new Error('404 page not showing');
    ...
    async testAccessibility() {
        ...
        await this.runTest('Screen reader labels', async () => {
            ...
            if (ariaLabels < 5)
                throw new Error('Insufficient ARIA labels for accessibility');
            await page.close();
            await context.close();
        });
    }
    ...
    async testSecurityFeatures() {
        ...
        await this.runTest('Protected routes', async () => {
            ...
            if (!currentUrl.includes('login'))
                throw new Error('Protected route not redirecting to login');
        ...
        await this.runTest('Admin route protection', async () => {
            ...
            if (adminContent > 0)
                throw new Error('Non-admin user accessing admin content');
            await page.close();
            await context.close();
        });
    }
    ...
    async testFormValidation() {
        ...
        await this.runTest('Email format validation', async () => {
            ...
            if (emailError === 0)
                console.log(chalk.gray('   Email validation message not shown'));
            await page.close();
            await context.close();
        });
    }
