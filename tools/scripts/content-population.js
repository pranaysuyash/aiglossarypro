...
export class ContentPopulator {
    ...
    async populateContent(inputPath, outputPath) {
    ...
    async loadTermsFromCSV(inputPath) {
    ...
    async enhanceTerm(term) {
        ...
        return {
            name: termName,
            mainCategory: term['Introduction – Category and Sub-category of the Term – Main Category'] ||
                this.inferMainCategory(termName, termType),
            subCategory: term['Introduction – Category and Sub-category of the Term – Sub-category'] ||
                this.inferSubCategory(termName, termType),
            definition,
            shortDefinition,
            keyPrinciples,
            importance: term['Introduction – Importance and Relevance in AI/ML'] ||
                this.generateImportance(termName, termType),
            applications,
            implementation,
            codeExamples,
            advantages,
    ...
    identifyTermType(termName) {
        const name = termName.toLowerCase();
        if (name.includes('network') || name.includes('neural')) {
            return 'network';
        }
        if (name.includes('algorithm') || name.includes('optimization')) {
            return 'algorithm';
        }
        ...
        if (name.includes('function') || name.includes('activation')) {
            return 'function';
        }
        if (name.includes('layer') || name.includes('architecture')) {
            return 'architecture';
        }
        return 'concept';
    }
    enhanceDefinition(termName, existing, termType) {
    ...
    generateDefinitionFromPattern(termName, pattern, termType) {
        const templates = {
    ...
    enhanceExistingDefinition(existing, termName, _termType) {
    ...
    generateShortDefinition(termName, fullDefinition, termType) {
    ...
    enhanceKeyPrinciples(termName, existing, termType) {
        if (existing && existing.length >= 200) {
            return existing;
        }
    ...
    enhanceApplications(termName, existing, _termType) {
        if (existing && existing.length >= 200) {
            return existing;
        }
    ...
    enhanceImplementation(termName, existing, _termType) {
    ...
    generateCodeExamples(termName, existing, termType) {
...
model.fit(X_train, y_train, epochs=10, validation_split=0.2)`,
    ...
    enhanceAdvantages(termName, existing, termType) {
        if (existing && existing.length >= 150) {
            return existing;
        }
    ...
    enhanceRelatedConcepts(termName, existing, termType) {
        if (existing && existing.length >= 150) {
            return existing;
        }
    ...
    generateInteractiveElements(termName, _termType) {
    ...
    inferMainCategory(_termName, termType) {
    ...
    inferSubCategory(termName, _termType) {
        const name = termName.toLowerCase();
        if (name.includes('supervised')) {
            return 'Supervised Learning';
        }
        if (name.includes('unsupervised')) {
            return 'Unsupervised Learning';
        }
        if (name.includes('reinforcement')) {
            return 'Reinforcement Learning';
        }
        if (name.includes('deep') || name.includes('neural')) {
            return 'Deep Learning';
        }
        if (name.includes('natural language')) {
        ...
        if (name.includes('clustering')) {
            return 'Clustering';
        }
        return 'General AI/ML';
    }
    generateImportance(termName, _termType) {
    ...
    generateTags(_termName, termType) {
    ...
    assessDifficultyLevel(termName, definition) {
        const name = termName.toLowerCase();
        const def = definition.toLowerCase();
        // Advanced indicators
        if (name.includes('advanced') ||
            name.includes('complex') ||
            def.includes('mathematical derivation') ||
            def.includes('theoretical')) {
            return 'Advanced';
        }
        // Expert indicators
        if (name.includes('research') ||
            def.includes('cutting-edge') ||
        ...
        if (name.includes('neural') ||
            name.includes('deep') ||
            def.includes('optimization') ||
            def.includes('algorithm')) {
            return 'Intermediate';
        }
        return 'Beginner';
    }
    calculateCompleteness(term) {
    ...
    async saveEnhancedTerms(terms, outputPath) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    populator.populateContent(inputPath, outputPath).catch(console.error);
}
