...
const validationType = getCLIArg('--type', 'all');
const targetTerm = getCLIArg('--term');
const shouldFix = args.includes('--fix');
const shouldReport = args.includes('--report');
const qualityThreshold = parseInt(getCLIArg('--threshold', '70'), 10);
/**
    ...
    try {
        ...
        for (const term of termsToValidate) {
            try {
                ...
                const result = await validateTerm(term, validationType);
 ...
 */
async function getTermsToValidate(targetTerm) {
    if (targetTerm) {
 ...
 */
async function validateTerm(term, validationType) {
    const issues = [];
 ...
 */
async function validateQuality(term) {
    const issues = [];
    const recommendations = [];
    let score = 100;
 ...
 */
async function validateCompleteness(term) {
    const issues = [];
    const recommendations = [];
    let score = 100;
    ...
    const enrichmentFields = [
        ...
        {
            field: 'mathFormulation',
            weight: 10,
            message: 'Mathematical formulation is missing (if applicable)',
        },
    ];
 ...
 */
async function validateConsistency(term) {
    const issues = [];
    const recommendations = [];
    let score = 100;
 ...
 */
async function autoFixIssues(term, fixableIssues) {
 ...
 */
async function regenerateDefinition(term) {
    try {
...
async function improveDefinition(term) {
    try {
...
async function generateShortDefinition(term) {
    if (!term.definition)
        return;
    try {
        ...
        if (shortDef.length > 20 && shortDef.length < 200) {
            await db.update(terms).set({ shortDefinition: shortDef }).where(eq(terms.id, term.id));
...
async function generateCharacteristics(term) {
    try {
        ...
        if (characteristics.length > 0) {
            await db.update(terms).set({ characteristics }).where(eq(terms.id, term.id));
...
async function generateApplications(term) {
    try {
        ...
        if (applications.length > 0) {
            await db.update(terms).set({ applications }).where(eq(terms.id, term.id));
...
async function checkTextIssues(text) {
    const issues = [];
    // Simple checks for common issues
    if (text.includes('  '))
        issues.push('Multiple consecutive spaces');
    if (text.match(/[a-z]\.[A-Z]/))
        issues.push('Missing space after period');
    if (text.match(/\b(the the|and and|of of|to to|in in)\b/))
...
async function checkDefinitionConsistency(definition, shortDefinition) {
    // Simple keyword overlap check
    const defWords = new Set(definition
        .toLowerCase()
        .split(/\s+/)
        .filter(word => word.length > 3));
    const shortWords = new Set(shortDefinition
        .toLowerCase()
        .split(/\s+/)
        .filter(word => word.length > 3));
...
async function checkCategoryConsistency(termName, definition, category) {
    // Check if term appears in essential terms for this category
    const essentialTerms = ESSENTIAL_AI_TERMS[category] || [];
    const isEssential = essentialTerms.some(t => t.name.toLowerCase() === termName.toLowerCase() ||
...
async function findSimilarTerms(termName, excludeId) {
    const similarTerms = await db
        .select()
        .from(terms)
        .where(and(not(eq(terms.id, excludeId)), like(terms.name, `%${termName.substring(0, Math.min(termName.length - 2, 10))}%`)))
        .limit(5);
    return similarTerms;
}
function checkTerminologyConsistency(term) {
    const issues = [];
...
function calculateSectionCompleteness(term) {
 ...
 */
function displayValidationSummary(report, duration) {
    const { summary } = report;
...
export { validateContent, validateTerm };
