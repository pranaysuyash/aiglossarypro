...
async function comprehensiveDataStatus() {
    ...
    `);
      ...
      FROM terms
    `);
        const tq = termsQuality.rows[0];
        const completeness = (((tq.total_terms - tq.missing_definitions) / tq.total_terms) *
            100).toFixed(1);
        console.log(`Total Terms: ${tq.total_terms.toLocaleString()}`);
        console.log(`Missing Definitions: ${tq.missing_definitions} (${((tq.missing_definitions / tq.total_terms) * 100).toFixed(1)}%)`);
      ...
      FROM enhanced_terms
    `);
        const eq = enhancedQuality.rows[0];
        console.log(`Total Enhanced Terms: ${eq.total_enhanced.toLocaleString()}`);
        console.log(`With Code Examples: ${eq.with_code_examples} (${((eq.with_code_examples / eq.total_enhanced) * 100).toFixed(1)}%)`);
        console.log(`With Interactive Elements: ${eq.with_interactive} (${((eq.with_interactive / eq.total_enhanced) * 100).toFixed(1)}%)`);
        console.log(`With Case Studies: ${eq.with_case_studies} (${((eq.with_case_studies / eq.total_enhanced) * 100).toFixed(1)}%)`);
        console.log(`With Implementation: ${eq.with_implementation} (${((eq.with_implementation / eq.total_enhanced) * 100).toFixed(1)}%)`);
        console.log(`With Categories: ${eq.with_categories} (${((eq.with_categories / eq.total_enhanced) * 100).toFixed(1)}%)`);
      ...
      `);
            const count = searchResults.rows[0].match_count;
      ...
      LIMIT 5
    `);
        sampleContent.rows.forEach((term, i) => {
    ...
    `);
        const api = apiReadiness.rows[0];
        console.log(`Searchable Terms: ${api.searchable_terms.toLocaleString()} ${api.searchable_terms > 1000 ? '✅' : '⚠️'}`);
        console.log(`Active Categories: ${api.active_categories} ${api.active_categories > 10 ? '✅' : '⚠️'}`);
        console.log(`Term Sections: ${api.term_sections} ${api.term_sections > 0 ? '✅' : '⚠️'}`);
        console.log(`Terms with Examples: ${api.terms_with_examples} ${api.terms_with_examples > 100 ? '✅' : '⚠️'}`);
        ...
        if (eq.with_code_examples / eq.total_enhanced < 0.1) {
            issues.push(`Low code examples coverage: ${((eq.with_code_examples / eq.total_enhanced) * 100).toFixed(1)}%`);
        }
