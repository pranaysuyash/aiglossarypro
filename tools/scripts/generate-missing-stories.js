...
class StoryGenerator {
    sourceDir = path.join(process.cwd(), 'client/src');
    componentsFound = [];
    storiesGenerated = 0;
    async run() {
    ...
    categorizeComponent(relativePath) {
        ...
        if (relativePath.includes('/components/'))
            return 'Components';
        if (relativePath.includes('/interactive/'))
            return 'Interactive';
        if (relativePath.includes('/ar/') || relativePath.includes('/vr/'))
            return 'AR/VR';
        if (relativePath.includes('/accessibility/'))
            return 'Accessibility';
        if (relativePath.includes('/analytics/'))
            return 'Analytics';
        return 'Components';
    }
    detectExportType(content) {
    ...
    async analyzeExistingStories() {
        const storyFiles = await glob('client/src/**/*.stories.*');
        const existingStoryComponents = new Set();
        ...
        // Group by category
        const byCategory = missingStories.reduce((acc, comp) => {
    ...
    async generateStoryFile(component) {
    ...
    componentNeedsDecorators(component) {
        // Components that likely need React Query or other providers
        return (component.name.toLowerCase().includes('dashboard') ||
    ...
    generateStoryVariants(component) {
        const baseStories = [
            {
                name: 'Default',
