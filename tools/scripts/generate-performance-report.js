...
|-----------|---------|----------|----------|--------------|
${componentStats
            .map(stat => `| ${stat.component} | ${stat.renderCount} | ${stat.avgRenderTime.toFixed(2)}ms | ${stat.maxRenderTime.toFixed(2)}ms | ${stat.avgMemoryUsage.toFixed(2)}MB |`)
            .join('\n')}
...
${this.generateInsights(summary, componentStats)}
    ...
    generateInsights(summary, componentStats) {
        const insights = [];
        ...
        const mostRenderedComponent = componentStats.reduce((max, comp) => (comp.renderCount > max.renderCount ? comp : max), componentStats[0]);
