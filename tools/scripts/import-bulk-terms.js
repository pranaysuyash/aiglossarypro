...
class BulkTermImporter {
    ...
    async importCSV() {
        ...
        try {
            // Process file
            await pipeline(createReadStream(this.options.source), parser, processor, { signal: this.abortController.signal });
        }
        finally {
    ...
    findColumnValue(record, possibleColumns) {
    ...
    parseArrayField(record, possibleColumns) {
    ...
    parseComplexityLevel(record) {
        const complexity = this.findColumnValue(record, ['Complexity', 'complexity_level', 'difficulty']);
        if (!complexity)
            return 'intermediate';
        const lower = complexity.toLowerCase();
        if (lower.includes('begin') || lower.includes('easy') || lower.includes('basic'))
            return 'beginner';
        if (lower.includes('adv') || lower.includes('hard') || lower.includes('expert'))
            return 'advanced';
        return 'intermediate';
    }
    parseImplementationStatus(record) {
        const status = this.findColumnValue(record, ['Status', 'implementation_status']);
        if (!status)
            return 'theoretical';
        const lower = status.toLowerCase();
        if (lower.includes('impl') || lower.includes('prod') || lower.includes('ready'))
            return 'implemented';
        if (lower.includes('exp') || lower.includes('beta') || lower.includes('test'))
            return 'experimental';
        return 'theoretical';
    }
    parse295ColumnSections(record, headers) {
    ...
    async importJSON() {
        ...
        for (let i = 0; i < terms.length; i += this.options.batchSize) {
            ...
            const parsedBatch = batch.map(term => this.parseJSONTerm(term)).filter(Boolean);
    ...
    parseJSONTerm(data) {
        try {
            if (!data.term_name && !data.name && !data.title)
                return null;
            return {
    ...
    async processTermsConcurrently(terms) {
        let successful = 0;
        let failed = 0;
    ...
    getSectionDisplayType(sectionName) {
