...
// Database connection
const connectionString = process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5432/aiglossary';
...
async function investigateDatabase() {
    ...
    try {
        ...
        console.log('='.repeat(50));
        const [basicCount] = await db.select({ count: sql `count(*)` }).from(terms);
        const [enhancedCount] = await db.select({ count: sql `count(*)` }).from(enhanced_terms);
