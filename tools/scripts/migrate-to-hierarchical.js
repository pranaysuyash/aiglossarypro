...
class HierarchicalMigrator {
    ...
    isDryRun = false;
    batchSize = 100;
    logFile = `migration-log-${Date.now()}.json`;
    migrationLog = [];
    ...
    async validateDatabaseState() {
        ...
        for (const table of requiredTables) {
            const result = await this.db.execute(sql `SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = ${table})`);
            if (!result.rows[0].exists) {
                throw new Error(`Required table '${table}' does not exist`);
            }
        }
        // Check for data integrity issues
        const orphanedSections = await this.db.execute(sql `SELECT COUNT(*) as count FROM sections s 
    ...
    async backupCurrentData() {
        ...
        for (const table of backupTables) {
            ...
            if (!this.isDryRun) {
                await this.db.execute(sql `CREATE TABLE ${sql.identifier(backupTableName)} AS SELECT * FROM ${sql.identifier(table)}`);
            }
    ...
    async getCurrentTerms() {
        ...
        const terms = await this.db.execute(sql `
        SELECT 
          et.id,
          et.name,
          et.slug,
     ...
     */
    async getCurrentSections(termId) {
        const sections = await this.db.execute(sql `
     ...
     */
    async getCurrentSectionItems(sectionIds) {
     ...
     */
    async getUserProgress(termId) {
        const progress = await this.db.execute(sql `
     ...
     */
    createHierarchicalStructure(term, currentSections, currentSectionItems) {
        ...
        const processNode = (node, path, depth, parentPath) => {
            // Find matching current section or create new one
            const existingSection = currentSections.find(s => s.name.toLowerCase() === node.name.toLowerCase() ||
     ...
     */
    migrateUserProgress(oldProgress, newSections) {
        const migratedProgress = [];
        // Create mapping from old sections to new hierarchical paths
        const _pathMapping = new Map();
        // For each old progress record, find the best matching new section
        oldProgress.forEach(progress => {
            const matchingSection = newSections.find(section => section.sectionData.items?.some((item) => item.originalSectionId === progress.sectionId));
            if (matchingSection) {
     ...
     */
    async deleteOldSections(termId) {
        await this.db.execute(sql `DELETE FROM sections WHERE term_id = ${termId}`);
    }
    /**
     * Insert new hierarchical sections
     */
    async insertHierarchicalSections(sections) {
     ...
     */
    async updateTermMetadata(termId, sections) {
        const hasInteractive = sections.some(s => s.isInteractive);
        const hasCodeExamples = sections.some(s => s.sectionData.items?.some((item) => item.contentType === 'code'));
     ...
     */
    async verifyMigrationIntegrity(results) {
        ...
        const orphanedSections = await this.db.execute(sql `
