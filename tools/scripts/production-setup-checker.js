...
class ProductionSetupChecker {
    ...
    async testPostHogConfiguration() {
    ...
    async checkGumroadConfiguration() {
    ...
    async runAllChecks() {
        ...
        for (const check of checks) {
            ...
            try {
                ...
                const status = result.status === 'PASS' ? 'success' : result.status === 'WARN' ? 'warn' : 'error';
