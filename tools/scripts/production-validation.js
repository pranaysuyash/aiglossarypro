...
class ProductionValidator {
    ...
    async runTest(testName, testFn) {
        ...
        try {
            ...
            const statusColor = result.status === 'PASS' ? 'success' : result.status === 'FAIL' ? 'error' : 'warn';
            this.log(`  ${statusIcon} ${result.test}: ${result.message} (${result.duration}ms)`, statusColor);
            return result;
        }
        catch (error) {
            const result = {
    ...
    // GA4 Analytics Tests
    async testGA4MeasurementProtocol() {
    ...
    async testGumroadWebhookSecurity() {
        ...
        try {
            ...
            const isValid = crypto.timingSafeEqual(Buffer.from(expectedSignature), Buffer.from(expectedSignature));
            if (isValid) {
                return {
                    service: 'Gumroad',
                    test: 'Webhook Security',
    ...
    async runValidation() {
        ...
        for (const suite of testSuites) {
            this.log(`\n🔧 Testing ${suite.name}${suite.required ? ' (Required)' : ' (Optional)'}...`, 'info');
    ...
    generateReport() {
        ...
        if (failed > 0) {
            ...
            this.log('❌ Some critical services failed validation. Please resolve issues before deploying.', 'error');
        }
        else if (skipped > 0) {
            this.log('\n⚠️ PRODUCTION READINESS: PARTIALLY READY', 'warn');
            this.log('✅ Core services are working, but some optional services are not configured.', 'warn');
        }
        else {
