...
class ProductionDeployment {
    ...
    async validateEnvironment() {
        ...
        if (totalMem < 2048) {
            console.warn('⚠️  Low memory allocation detected. Consider increasing with --max-old-space-size=4096');
        }
    ...
    async installDependenciesAndBuild() {
    ...
    async processContent() {
        ...
        try {
            ...
            if (this.config.environment === 'production') {
                ...
                this.log(`⚠️  Content processing failed (non-critical in ${this.config.environment}): ${error}`);
            }
        }
    }
        ...
        try {
            ...
            const securityChecks = [
                ...
                },
                {
                    ...
                    check: () => this.config.environment === 'production' ? process.env.NODE_ENV === 'production' : true,
    ...
    async validateContent() {
        ...
        try {
            ...
            const { sql } = await import('drizzle-orm');
            // Get counts
            const [termsCount] = await db.select({ count: sql `count(*)` }).from(terms);
            const [categoriesCount] = await db.select({ count: sql `count(*)` }).from(categories);
            const [enhancedCount] = await db.select({ count: sql `count(*)` }).from(enhancedTerms);
            const [sectionsCount] = await db.select({ count: sql `count(*)` }).from(termSections);
            ...
            if (categoriesCount.count < minCategories) {
                throw new Error(`Insufficient categories: ${categoriesCount.count} (minimum: ${minCategories})`);
            }
...
async function checkForCSVFile() {
    try {
...
async function runProductionDeployment() {
    ...
    const enablePerformanceOptimizations = args.includes('--optimize') || environment === 'production';
