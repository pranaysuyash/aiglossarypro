...
class QuickSecurityTest {
  ...
  private testSecurityImplementation(): void {
    ...
    try {
      const xssFiles = execSync(
        'find ./client ./server -name "*.ts" -o -name "*.tsx" | xargs grep -l "sanitize\\|DOMPurify" | wc -l',
        { encoding: 'utf8' }
      ).trim();
    ...
    try {
      const rateLimitFiles = execSync(
        'find ./server -name "*.ts" | xargs grep -l "rateLimit\\|express-rate-limit" | wc -l',
        { encoding: 'utf8' }
      ).trim();
    ...
    try {
      const authFiles = execSync(
        'find ./server -name "*.ts" | xargs grep -l "firebase\\|jwt" | wc -l',
        { encoding: 'utf8' }
      ).trim();
  ...
  private testDatabaseSecurity(): void {
    ...
    try {
      const drizzleFiles = execSync(
        'find ./server -name "*.ts" | xargs grep -l "drizzle" | wc -l',
        { encoding: 'utf8' }
      ).trim();
  ...
  private testSecurityConfiguration(): void {
    ...
    try {
      const validationFiles = execSync(
        'find ./server -name "*.ts" | xargs grep -l "zod\\|joi\\|validator" | wc -l',
        { encoding: 'utf8' }
      ).trim();
