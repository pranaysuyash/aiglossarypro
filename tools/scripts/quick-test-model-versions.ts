...
async function quickTest() {
  ...
  try {
    ...
    const versions = await db.select().from(modelContentVersions).limit(5);
    ...
    if (versions.length > 0) {
      ...
      versions.forEach((version, index) => {
        console.log(
          `  ${index + 1}. Term: ${version.termId.substring(0, 8)}..., Model: ${version.model}, Section: ${version.sectionName}`
        );
      });

      // Test the getModelVersions function from the service
      const aiContentGenerationService = await import(
      ...
      const serviceVersions =
        await aiContentGenerationService.aiContentGenerationService.getModelVersions(
          testTermId,
          testSectionName
        );
      ...
      serviceVersions.forEach((version, index) => {
        console.log(
          `  ${index + 1}. Model: ${version.model}, Cost: $${version.cost}, Tokens: ${version.totalTokens}`
        );
      });
    }
  } catch (error) {
