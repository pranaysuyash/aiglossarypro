...
class QuickVisualAuditor {
    ...
    resultsDir = path.join(process.cwd(), 'visual-audit-results', this.timestamp);
    results = [];
    async initialize() {
    ...
    async testPage(page, pageName, breakpoint) {
        const screenshotPath = path.join(this.resultsDir, 'screenshots', `${pageName}-${breakpoint}.png`);
        try {
    ...
    async checkAccessibility(page) {
        ...
        const buttonsWithoutText = await page.$$eval('button', buttons => buttons.filter(btn => !btn.textContent?.trim() && !btn.getAttribute('aria-label')).length);
        ...
        const inputsWithoutLabels = await page.$$eval('input:not([type="hidden"])', inputs => inputs.filter(input => {
            const id = input.id;
            if (!id)
                return true;
    ...
    async testAuthentication(page) {
        ...
        try {
            ...
            if (!emailInput || !passwordInput || !submitButton) {
                throw new Error(`Login form elements not found. Email: ${!!emailInput}, Password: ${!!passwordInput}, Submit: ${!!submitButton}`);
            }
            ...
            // Logout
            const logoutButton = await page.$('[data-testid="logout"], button:has-text("Logout"), button:has-text("Sign out")');
            if (logoutButton) {
    ...
    async testInteractions(page) {
        ...
        try {
            ...
            // Test theme toggle
            const themeToggle = await page.$('[data-testid="theme-toggle"], button[aria-label*="theme"], button:has-text("Theme")');
            if (themeToggle) {
            ...
            const menuButton = await page.$('[data-testid="menu-button"], button[aria-label*="Menu"], .hamburger');
            if (menuButton) {
            ...
            if (categoryFilter) {
                const options = await page.$$eval('select[name="category"] option', opts => opts.map(opt => opt.value));
                if (options.length > 1) {
    ...
    generateResultsHTML(results) {
      ...
      <div class="result-item">
        <div class="result-header">
          <h3>${page}</h3>
          <span>${pageResults.filter(r => !r.error).length} / ${pageResults.length} passed</span>
        </div>
        ...
        `
            : ''}
        ${pageResults.some(r => r.issues && r.issues.length > 0)
            ? `
          <div class="issues">
            ...
            ${pageResults
                .filter(r => r.issues)
                .map(r => r.issues
                .map((issue) => `${r.breakpoint}: ${issue.type} (${issue.count || issue.issues?.length || 0})`)
                .join('<br>'))
                .join('<br>')}
          </div>
        `
