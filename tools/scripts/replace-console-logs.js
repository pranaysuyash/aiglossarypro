...
class ConsoleLogReplacer {
    ...
    serverFileExtensions = ['.ts', '.js'];
    clientFileExtensions = ['.tsx', '.jsx', '.ts', '.js'];
    totalFiles = 0;
    totalReplacements = 0;
    skippedFiles = 0;
    results = [];
    async replaceInProject(dryRun = false) {
    ...
    processDirectory(dir, type, dryRun) {
        try {
    ...
    shouldProcessFile(fileName, type) {
    ...
    processFile(filePath, type, dryRun) {
        this.totalFiles++;
        try {
