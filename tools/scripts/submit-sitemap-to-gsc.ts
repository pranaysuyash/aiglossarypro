...
class GoogleSearchConsoleSubmitter {
  ...
  private config: SitemapSubmissionConfig;

  constructor(config: SitemapSubmissionConfig) {
    this.config = config;
  }
    ...
    if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
      ...
      if (this.config.delegatedEmail) {
        return auth.getClient().then(client => {
          (client as any).subject = this.config.delegatedEmail;
          return client;
        });
      }
   ...
   */
  async listSitemaps(): Promise<any[]> {
    try {
   ...
   */
  async verifySitemapAccessibility(sitemapUrl: string): Promise<boolean> {
    try {
 ...
 */
async function main() {
  const config: SitemapSubmissionConfig = {
