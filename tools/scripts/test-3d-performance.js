...
class Performance3DGraphTester {
     ...
     */
    testLayoutCalculation(nodes, _edges) {
        const startTime = performance.now();
     ...
     */
    testRenderingPerformance(nodes, edges) {
        const startTime = performance.now();
     ...
     */
    estimateMemoryUsage(nodes, edges) {
     ...
     */
    calculateFrameRate(renderingTime, frames = 60) {
    ...
    saveResults() {
        ...
        try {
            writeFileSync(join(outputDir, 'performance-test-results.json'), JSON.stringify(jsonResults, null, 2));
