...
class TestCoverageAnalyzer {
  ...
  private readonly reportDir = join(process.cwd(), 'coverage-reports');
  ...
  private generateMinimalReport(): CoverageReport {
    // Count test files
    let testCount = 0;
  ...
  private displayCoverageReport(coverage: CoverageReport): void {
    ...
    console.log(
      chalk.white.bold('Metric'.padEnd(15)) +
    ...
    metrics.forEach(metric => {
      ...
      console.log(
        chalk.white(metric.charAt(0).toUpperCase() + metric.slice(1).padEnd(15)) +
    ...
    if (coverage.files && Object.keys(coverage.files).length > 0) {
      ...
      if (uncoveredFiles.length > 0) {
        ...
        uncoveredFiles.forEach(([file, data]) => {
          ...
          console.log(
            chalk.red(`  ${data.lines.pct.toFixed(1)}%`.padEnd(8)) +
  ...
  private checkThresholds(coverage: CoverageReport): boolean {
    const metrics = ['lines', 'statements', 'functions', 'branches'] as const;
