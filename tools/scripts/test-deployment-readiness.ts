...
const checks: Check[] = [
    ...
    check: async () => {
      try {
        ...
        const result = execSync(
          `grep -r -i -E "${patterns.join('|')}" --exclude-dir=node_modules --exclude-dir=.git --exclude="*.env*" . || true`,
          { encoding: 'utf8' }
        );
...

  {
    ...
    check: async () => {
      ...
      if (hasPostHog || hasGA) return true;
    ...
    check: async () => {
      try {
        const imageFiles = execSync(
          'find apps/web/public -name "*.png" -o -name "*.jpg" -o -name "*.jpeg" | wc -l',
          { encoding: 'utf8' }
        ).trim();
  ...
  // Payment Integration
  {
    ...
    check: async () => {
      ...
      if (hasKeys && hasWebhook) return true;
      if (hasKeys && !hasWebhook) return 'Stripe webhook not configured';
...
function generateReport() {
  ...
  if (failed > 0) {
    ...
    results
      ...
      .forEach(r => {
        console.log(chalk.red(`\n  ${r.category} - ${r.name}`));
        if (r.message) console.log(chalk.gray(`    Issue: ${r.message}`));
        if (r.fix) console.log(chalk.yellow(`    Fix: ${r.fix}`));
      });
  }
