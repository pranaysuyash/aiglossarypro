...
const results = [];
function logTest(test, status, message, details) {
...
async function testPricingAPI() {
    ...
    try {
        ...
        if (statusRes.ok && statusData.success) {
            logTest('GET /api/pricing/phase-status', 'passed', `Current phase: ${statusData.data.currentPhase}`, statusData.data);
        }
        else {
        ...
        if (salesRes.ok && salesData.success) {
            logTest('GET /api/pricing/sales-count', 'passed', `Total sales: ${salesData.data.totalSales}`);
        }
        else {
        ...
        if (transitionRes.status === 403 || transitionRes.status === 401) {
            logTest('POST /api/pricing/transition-phase (auth check)', 'passed', 'Correctly requires authentication');
        }
        else {
            logTest('POST /api/pricing/transition-phase (auth check)', 'warning', 'Should require authentication');
        }
    }
    catch (error) {
...
async function testPricingDisplay() {
    ...
    try {
        ...
        const priceElements = await page.$$eval('.text-3xl.font-bold.text-purple-900', elements => elements.map(el => el.textContent));
        if (priceElements.length > 0) {
            ...
            if (prices.some(p => p?.includes(expectedPrices[currentPhase]))) {
                logTest('Phase-specific pricing', 'passed', `Showing ${currentPhase} phase price: ${expectedPrices[currentPhase]}`);
            }
            else {
                logTest('Phase-specific pricing', 'warning', `Expected ${expectedPrices[currentPhase]} for ${currentPhase} phase`);
            }
        }
        else {
        ...
        const discountElements = await page.$$eval('.line-through', elements => elements.map(el => el.textContent));
    ...
    try {
        ...
        if (posthogLoaded) {
            ...
            if (ctaButton) {
                ...
                const trackingEvents = networkRequests.filter(url => url.includes('event') || url.includes('capture'));
                if (trackingEvents.length > 0) {
                    logTest('Event tracking', 'passed', `${trackingEvents.length} tracking events captured`);
                }
                else {
...
async function testMobileExperience() {
    ...
    try {
        ...
        if (!viewportTest.hasHorizontalScroll) {
            ...
            logTest('Mobile responsive design', 'failed', 'Horizontal scroll detected', viewportTest);
        }
        // Check touch targets
        ...
        if (invalidButtons.length === 0) {
            ...
            logTest('Mobile touch targets', 'warning', `${invalidButtons.length} buttons too small`, invalidButtons);
        }
