...
class TestQualityChecker {
  ...
  private coverageReport: TestCoverageReport = {
    totalFiles: 0,
    testedFiles: 0,
    untestedFiles: [],
  ...
  private async analyzeCoverage() {
    ...
    const criticalGaps = untestedFiles.filter(file => 
  ...
  private getPossibleSourceFiles(testFile: string): string[] {
  ...
  private generateRecommendations(criticalGaps: string[], untestedFiles: string[]): string[] {
    const recommendations: string[] = [];

    if (criticalGaps.length > 0) {
      recommendations.push(
        `🚨 Critical: Add tests for ${criticalGaps.length} security/business-critical files`
      );
    }
    ...
    if (serviceFiles.length > 5) {
      recommendations.push(
        `⚠️  High Priority: ${serviceFiles.length} service files lack tests`
      );
    }
    ...
    if (componentFiles.length > 50) {
      recommendations.push(
        `📊 Medium Priority: ${componentFiles.length} React components need tests`
      );
    }

    if (this.coverageReport.coveragePercentage < 50) {
      recommendations.push(
        '🎯 Goal: Aim for at least 80% test coverage for critical paths'
      );
    }
  ...
  private async analyzeTestFile(filePath: string): Promise<TestQualityMetrics> {
  ...
  private generateReport() {
    ...
    const coverageColor = 
      this.coverageReport.coveragePercentage >= 80 ? chalk.green :
  ...
  private getDirectoryCounts(files: string[]): Record<string, number> {
