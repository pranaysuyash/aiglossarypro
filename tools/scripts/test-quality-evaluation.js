...
async function testComparisonEvaluation() {
    ...
    try {
        const comparisonResult = await aiQualityEvaluationService.compareWithReference(SAMPLE_CONTENT.poorDefinition, SAMPLE_CONTENT.goodDefinition, 'definition');
...
async function testQualityAnalytics() {
    ...
    try {
        // Get analytics
        const analytics = await qualityAnalyticsService.generateQualityReport(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), new Date(), { groupBy: 'category' });
...
export { testSingleEvaluation, testBatchEvaluation, testComparisonEvaluation, testTemplateSystem, testQualityAnalytics, testRealTermEvaluation, runComprehensiveTest, };
