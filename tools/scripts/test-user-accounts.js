...
class UserAccountTester {
    ...
    screenshotDir = path.join(process.cwd(), 'user-account-tests');
    constructor() {
    ...
    async testFeatureAccess(page, user, result) {
    ...
    async testRestrictedFeature(page, feature) {
        try {
            switch (feature) {
                case 'admin-panel':
