...
class TypeScriptErrorFixer {
    ...
    private groupErrorsByType(errors: TypeScriptError[]): Map<string, TypeScriptError[]> {
    ...
    private async fixCommonErrors(errorsByType: Map<string, TypeScriptError[]>): Promise<void> {
    ...
    private async fixCannotFindNameErrors(errors: TypeScriptError[]): Promise<void> {
    ...
    private async fixArgumentTypeErrors(errors: TypeScriptError[]): Promise<void> {
    ...
    private async fixTypeAssignmentErrors(errors: TypeScriptError[]): Promise<void> {
    ...
    private async fixImplicitAnyErrors(errors: TypeScriptError[]): Promise<void> {
    ...
    private async fixPropertyNotExistErrors(errors: TypeScriptError[]): Promise<void> {
    ...
    private async addTypeAssertion(error: TypeScriptError): Promise<void> {
        try {
    ...
    private addTypeAssertionToLine(line: string, error: TypeScriptError): string {
    ...
    private async fixImplicitAnyInFile(filePath: string, errors: TypeScriptError[]): Promise<void> {
        try {
    ...
    private addTypeToImplicitAny(line: string, error: TypeScriptError): string {
    ...
    private async addOptionalChaining(error: TypeScriptError): Promise<void> {
        try {
            const filePath = path.resolve(error.file);
            if (!fs.existsSync(filePath)) return;
    ...
    private addOptionalChainingToLine(line: string, error: TypeScriptError): string {
    ...
    private reportRemainingErrors(): void {
        try {
            execSync('npx tsc --noEmit', { stdio: 'pipe' });
