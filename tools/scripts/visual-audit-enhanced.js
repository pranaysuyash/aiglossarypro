...
class EnhancedVisualAuditor {
        ...
        return [
            ...
            // Mobile Tests
            {
                ...
                actions: [
                    ...
                    {
                        type: 'click',
                        selector: 'button[aria-label*="menu"], button[aria-label*="navigation"], .mobile-menu-toggle, .hamburger, button:has(svg)',
            ...
            // Search Interaction Test
            {
                ...
                actions: [
                    {
                        type: 'click',
                        selector: 'input[type="text"], input[placeholder*="Search"], input[placeholder*="search"], .search-input',
            ...
            // Terms Listing with Filters
            {
                ...
                actions: [
                    ...
                    {
                        type: 'click',
                        selector: 'button:contains("Filters"), button[aria-label*="filter"], .filter-button, button:has([data-lucide="filter"])',
    ...
    async performAction(page, action, config) {
        ...
        switch (action.type) {
            ...
            case 'focus':
                ...
                break;
            case 'screenshot': {
                const screenshotPath = path.join(this.screenshotDir, 'interactions', `${config.name}-${action.description?.replace(/\s+/g, '-') || Date.now()}.png`);
        ...
        if (action.screenshot && action.type !== 'screenshot') {
            const screenshotPath = path.join(this.screenshotDir, 'interactions', `${config.name}-${action.type}-${Date.now()}.png`);
            await page.screenshot({ path: screenshotPath });
        }
    }
    async testComponent(page, component, config) {
        ...
        try {
            ...
            for (const interaction of component.interactions) {
                ...
                const screenshotPath = path.join(this.screenshotDir, 'components', `${config.name}-${component.name}-${interaction.type}.png`);
            ...
            const fallbackScreenshotPath = path.join(this.screenshotDir, 'components', `${config.name}-${component.name}-fallback.png`);
            await page.screenshot({ path: fallbackScreenshotPath });
        }
    }
    async testState(page, state, config) {
    ...
    async runAssertion(page, assertion, config, state) {
        try {
    ...
    async runAccessibilityTests(page, config) {
    ...
    async testFocusVisibility(page, config) {
    ...
    async testKeyboardNavigation(page, config) {
        // Test tab order
        const tabOrder = [];
        for (let i = 0; i < 10; i++) {
    ...
    async testColorContrast(page, config) {
    ...
    async runPerformanceAnalysis(page, config) {
    ...
    async analyzeScreenshot(screenshotPath, pageName) {
    ...
    async analyzeComponentScreenshot(_screenshotPath, _componentName, _interaction) {
...
<body>
    ...
    async getAllScreenshots() {
        const screenshots = [];
        const addScreenshots = async (dir, prefix = '') => {
